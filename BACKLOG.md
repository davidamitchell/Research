...
A new static page `docs/backlog.html` lists all items currently in `Research/backlog/`. Each item renders as a `.card` component (matching `all-items.html`) showing the item title, date, tags, and research question excerpt. A nav link labelled "Backlog" is added to the shared `nav-links` block in `scripts/build_site.py` alongside the existing "All items", "Browse", and "Tags" links. The link is active when on `backlog.html`.

The page is generated by `scripts/build_site.py` at build time by reading frontmatter and the `## Research Question` section from each file in `Research/backlog/`.

### Context

Outstanding research questions are only visible by browsing the repo. The site shows completed research but gives no visibility into what is queued. This page closes that gap using components and nav patterns already established in the site.

References:
- `docs/all-items.html` — card grid and nav pattern to match
- `scripts/build_site.py` — site build script to extend
- `Research/backlog/` — source directory
