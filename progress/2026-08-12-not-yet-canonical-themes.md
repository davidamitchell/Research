...
- `make check` (full repo) → now clean after formatting Python code blocks in four pre-existing `Research/completed/` items (`2026-02-28-transcript-via-gemini-api.md`, `2026-02-28-transcript-via-third-party-apis.md`, `2026-03-02-chat-conversational-interface.md`, `2026-03-08-slack-bot-memory-capture-retrieval.md`). These were pure formatting changes inside fenced code blocks, not changes to findings, claims, or conclusions, so no `versions:` entry was required under ADR-0013.
...
2. **What slowed down or went wrong?**
   ...
   - **`make check` scope:** The original agent reported that `make check` passed "for modified files" but noted remaining unformatted files in `Research/completed/`. The instructions require `make check` (full repo: `ruff check .` + `ruff format --check .`) to pass before pushing. Those four pre-existing formatting violations in completed research items meant the full-repo check was not clean. They have now been fixed as part of this cleanup.
