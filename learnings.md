...
**Open thread:** What is the minimal viable formal layer for a given domain? The intent alignment item shows that full formal verification is impractical at scale. What is the right *combination* of formal + informal for a software team? For an agentic system? The new item suggests a concrete starting point: extend Hoshin Kanri X-matrix with typed causal-link annotations (+/−) and a lightweight constraint solver — producing consistency checking without requiring a new formal paradigm. The Quint ecosystem adds a further concrete starting point for software teams: executable specifications as the practical mid-level formal layer, with LLM translation reducing authorship cost.
...
*Last updated: 2026-03-16 (Thread 4 extended: cognitive debt concept, Quint LLM workflow as practical formal-spec instantiation, Spectacle as negative case study; Thread 11 extended: cognitive debt as trust-deficit dimension of verification bottleneck, practitioner 10–20× tokens evidence)*
