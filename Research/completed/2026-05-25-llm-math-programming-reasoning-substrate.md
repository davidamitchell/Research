...
- 2e. What do harder real-world benchmarks (Software Engineering bench, SWE-bench) indicate about actual code generation capability?
...
7. Training data domain distribution is a confounding factor: Minerva 62B achieves 50.3% on the MATH dataset versus PaLM-540B's 8.8%, suggesting that domain-specific mathematical pre-training contributes more to performance than raw model scale in this pairing. ([inference]; medium confidence; source: https://arxiv.org/abs/2206.14858; https://arxiv.org/abs/2402.00157)
...
The second mechanism, compositional structure alignment with CoT, is real but secondary. [inference; source: https://arxiv.org/abs/2201.11903] The CoT benefit is largest in mathematics precisely because mathematical solutions decompose naturally into explicit intermediate steps, which is also how human-written mathematical texts are structured. This is at least partly a training data artifact: LLMs trained on textbooks and worked solutions generate step-by-step reasoning because the domain is documented that way, not necessarily because they have internalized formal mathematical logic. [inference; source: https://arxiv.org/abs/2201.11903; https://arxiv.org/abs/2402.00157]

The third mechanism, external verifiability, is operationally distinctive among the three: it alone provides a pre-deployment correction loop that works regardless of model confidence. [inference; source: https://davidamitchell.github.io/Research/research/2026-04-26-llm-verifiability-asymmetry-code-world-action.html; https://deepmind.google/blog/ai-solves-imo-problems-at-silver-medal-level/] Compilers, type checkers, and test suites provide a correction loop that does not exist for natural language or world action outputs. AlphaProof confirms this: the path to IMO-level mathematics runs through Lean's formal proof checker as a reward signal, not through informal generation.
...
comparative_claim_audit: finding 3 reframed to "qualitatively distinct operational advantage" - comparative superlative removed
