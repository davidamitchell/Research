...
- [fact] Lewis et al. define Retrieval-Augmented Generation as language generation conditioned on retrieved passages from a dense vector index. [source: https://arxiv.org/abs/2005.11401]
- [inference] Because retrieved passages are injected into generation at inference time, retrieved documents function as part of the model's effective input rather than as a background reference. [source: https://arxiv.org/abs/2005.11401]
...
- [fact] Azure AI Search distinguishes indexed knowledge sources from remote knowledge sources. [source: https://learn.microsoft.com/en-us/azure/search/search-what-is-azure-search]
- [inference] A deployed agent can therefore depend either on a refreshed local index or on live remote retrieval, and those dependency modes carry different rollback and reproducibility properties. [source: https://learn.microsoft.com/en-us/azure/search/search-what-is-azure-search]
...
- [fact] Gao et al. frame RAG as a response to hallucination, outdated knowledge, and non-transparent reasoning in Large Language Models. [source: https://arxiv.org/abs/2312.10997]
- [inference] That framing makes freshness and traceability central rather than peripheral RAG concerns. [source: https://arxiv.org/abs/2312.10997]
- [fact] Ragas states that RAG performance is affected by the retrieval model, the corpus, the LLM, and the prompt formulation. [source: https://arxiv.org/html/2309.15217v2; https://docs.ragas.io/en/stable/]
- [inference] Because the corpus is one of the stated performance variables, corpus changes can alter measured quality even when the model and prompt stay fixed. [source: https://arxiv.org/html/2309.15217v2; https://docs.ragas.io/en/stable/]
...
- [fact] Azure AI Search's retrieval architecture returns not only retrieved text but also source references and execution metadata. [source: https://learn.microsoft.com/en-us/azure/search/agentic-retrieval-overview]
- [inference] A corpus change can therefore alter citation surface and provenance even when the answer text still looks plausible. [source: https://learn.microsoft.com/en-us/azure/search/agentic-retrieval-overview]
...
- [fact] The Ragas `rag_eval` template creates local datasets, experiment runs, logs, and Comma-Separated Values (CSV) outputs for repeated execution of the same test set against a RAG application. [source: https://docs.ragas.io/en/latest/howtos/cli/rag_eval/]
...
- [fact] Elasticsearch aliases allow applications to change which index they query in real time, and the aliases Application Programming Interface (API) supports atomic remove-plus-add swaps with no downtime. [source: https://www.elastic.co/guide/en/elasticsearch/reference/current/aliases.html]
...
- [fact] Azure AI Search indexers use change tracking to resume from the last successfully indexed document. [source: https://learn.microsoft.com/en-us/azure/reliability/reliability-ai-search; https://learn.microsoft.com/azure/search/search-howto-index-changed-deleted-blobs?tabs=portal]
- [inference] Index refresh is therefore a managed pipeline step rather than an implicit guarantee. [source: https://learn.microsoft.com/en-us/azure/reliability/reliability-ai-search; https://learn.microsoft.com/azure/search/search-howto-index-changed-deleted-blobs?tabs=portal]
...
- [inference] Foundational RAG sources and current Azure platform docs align on the same propagation mechanism: retrieved external content is added to model input at inference time. [source: https://arxiv.org/abs/2005.11401; https://learn.microsoft.com/en-us/azure/foundry/concepts/retrieval-augmented-generation; https://learn.microsoft.com/en-us/azure/search/search-what-is-azure-search]
...
- Review result: latest review-pass violations fixed; ready for re-review
