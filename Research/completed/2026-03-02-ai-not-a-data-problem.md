...
- [x] NIST AI RMF (AI Risk Management Framework): https://www.nist.gov/artificial-intelligence/ai-risk-management-framework — governance and organisational accountability
...
- C4: What does the CDO/CDAO role evolution show about the structural limits of data-team AI ownership?
...
The evidence converges on a single structural conclusion from three independent directions. First, the technical/architectural direction: the operational vs. analytical data plane distinction is a foundational architectural principle (Fowler, data mesh) and the NFR gap between analytics platforms and production agent requirements is unbridgeable by incremental configuration. Second, the organisational direction: regulatory frameworks and empirical failure data converge on the same conclusion — AI governance is inherently cross-functional and no data/analytics team can hold it unilaterally. Third, the evidence direction: the 74% failure rate statistic, the HBR CDO failure analysis, and the IBM CDO readiness gap are three independent empirical data points that document the outcome when data teams try to own production AI.
