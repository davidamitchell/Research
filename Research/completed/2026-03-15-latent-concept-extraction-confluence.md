...
added: 2026-03-16T09:16:52+00:00
status: completed
...
output: [knowledge]
themes: [rag-retrieval, knowledge-graphs, knowledge-management, ai-architecture]
...
**Economic lens:** VDB infrastructure costs are modest at Confluence scale. [assumption] A 10,000-page Confluence wiki with ~2,000 tokens/page = 20M tokens; at published embedding model batch processing costs (e.g. OpenAI text-embedding-3-small at $0.02/1M tokens, see platform.openai.com/docs/models/embeddings), a full initial embedding runs under $1. The primary cost is engineering implementation and ongoing governance (human review of deprecated/stale content). [inference] This economic asymmetry (low compute cost, high governance cost) explains the observed pattern of organisations over-investing in indexing technology and under-investing in knowledge governance.
...
**Historical lens:** The challenge of extracting structured knowledge from prose is not new: early knowledge management systems (late 1990s: Documentum, early 2000s: Verity, 2010s: Endeca) each attempted structured extraction from unstructured content. [assumption] Each generation encountered the same fundamental constraint: extraction quality is bounded by source content quality - garbage in, garbage out. Justification: this constraint follows directly from the nature of information extraction; it is not specific to any era or technology and is explicitly confirmed for the RAG/LLM era by the RULER benchmark and the prior RAG research item. BERTopic and mREBEL are better tools, but the fundamental constraint remains. The epistemics question is not a new research problem dressed in new terminology; it is the same problem that knowledge management has always faced, now with LLM-scale retrieval as the stakes.
