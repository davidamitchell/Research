...
**[fact]** The YAML front-matter standard for markdown notes in the Memory-System requires, at minimum, `date`, `title`, and `type` fields for classified files. These can all be generated by the triage agent. Source: Memory-System repository conventions (inferred from backlog item context and the existing research-loop item template structure used in this repo).
...
The workflow structure — checkout, install, agent invocation, commit, push — is identical in both use cases. The agent's task changes (classify inbox files instead of complete a research item), and the scheduling changes (every 6 hours instead of daily at 7am). Infrastructure and tooling are identical. The research-loop.yml has been operating reliably; the inbox triage workflow inherits that operational track record.
...
| Few-shot prompting raises accuracy substantially | Springer LLM review; AAAI ICWSM prompt study | medium | Direction consistent; exact delta varies |
...
The inbox pattern is the right design for this context. The friction it removes is real and documented independently by three literatures (HCI research, GTD methodology, PKM practice). The automation it depends on is achievable with existing infrastructure. The accuracy bar is not "perfect classification" but "better than a rushed user," and misclassifications are recoverable.
...
3. Mobile HCI research (CELDA 2023; Computers in Human Behavior 2020) provides two independent confirmations that each additional decision step increases cognitive load in time-pressured mobile contexts, directly supporting the friction-reduction claim.
...
| Few-shot prompting substantially raises accuracy | Springer LLM classification review; AAAI ICWSM prompt study | medium | Consistent directional finding; exact improvement varies by task |
...
The inbox pattern is the correct design. It resolves the friction problem structurally rather than by optimising the capture UI, because the root cause is the decision itself, not the interface that presents it. Three independent literatures — HCI, GTD, and PKM — converge on the same principle: classification decisions made under time pressure and incomplete context are lower quality and higher cost than decisions made in a scheduled, context-rich processing pass. Automating the processing pass (replacing human review with an agent) removes the only residual friction.

The central trade-off is between triage accuracy and operational simplicity. Higher accuracy requires a more complex prompt (more examples, more explicit definitions), which is harder to maintain. Lower accuracy increases manual correction burden. The design choice here — few-shot prompting with a `?-` fallback for genuine ambiguity — sits at the appropriate point on this curve: it optimises accuracy for the common cases while making failures visible and recoverable rather than silent or catastrophic.
