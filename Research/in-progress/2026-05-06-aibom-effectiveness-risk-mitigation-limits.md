...
# What security and governance risks can an Artificial Intelligence Bill of Materials (AIBOM) realistically mitigate for agentic, tool-using, stateful Artificial Intelligence (AI) workloads, and where does it create false assurance?

## Research Question

What categories of security and governance risk can an Artificial Intelligence Bill of Materials (AIBOM), both declared design-time and runtime-observed variants, realistically mitigate for agentic, tool-using, stateful Artificial Intelligence (AI) workloads, what metrics can quantify that mitigation, and where does an apparently complete AIBOM create dangerous false assurance by leaving behavioral, inferential, and emergent risks invisible?
...
3. **False assurance scenarios:** Document concrete scenarios where a complete, well-formed AIBOM provides false assurance, including prompt injection, Retrieval-Augmented Generation poisoning, memory poisoning, and legitimate-tool misuse.
4. **Inference opacity boundary:** Analyze where neural-network inference opacity creates an irreducible visibility gap that no AIBOM schema can close.
...
- [x] [Anthropic (2026) Claude Opus 4.6 System Card](https://www.anthropic.com/claude-opus-4-6-system-card) - official model-system-card evidence on interpretability tooling, reasoning-faithfulness limits, and residual unexpected behavior
...
- [fact; source: https://owasp.org/www-project-top-10-for-large-language-model-applications/] OWASP treats insecure plugin design and excessive agency as separate failure classes, which means a tool can be correctly declared and still create severe downstream risk when its invocation or authorization boundary is weak.
...
#### D. Inference opacity and irreducible visibility limits
...
- [fact; source: https://www.anthropic.com/claude-opus-4-6-system-card] Anthropic says it uses interpretability methods such as activation oracles, attribution graphs, and sparse autoencoder features to investigate model behavior, while also stating that its methods and tools for alignment evaluation continue to develop and that its ability to assess reasoning faithfulness in depth remains limited.
...
- [inference; source: https://www.nist.gov/itl/ai-risk-management-framework; https://www.anthropic.com/claude-opus-4-6-system-card] The irreducible boundary is that an AIBOM can inventory boundary conditions, observed context, and governance artifacts, but it cannot turn a frontier model's internal reasoning into a fully inspectable or replayable object.
...
- [inference; source: https://www.nist.gov/itl/ai-risk-management-framework; https://www.anthropic.com/claude-opus-4-6-system-card] The opacity boundary is not a temporary paperwork gap alone, because even the strongest current governance and interpretability sources still treat risk measurement and reasoning-faithfulness assessment as incomplete.
...
- [inference; source: https://arxiv.org/abs/2302.12173; https://arxiv.org/abs/2211.09527; https://www.microsoft.com/en-us/security/blog/2025/04/24/new-whitepaper-outlines-the-taxonomy-of-failure-modes-in-ai-agents/] An AIBOM creates false assurance when its completeness is mistaken for semantic safety, because prompt injection, Retrieval-Augmented Generation poisoning, memory poisoning, and legitimate-tool misuse can all succeed through correctly declared channels.
- [inference; source: https://www.nist.gov/itl/ai-risk-management-framework; https://www.anthropic.com/claude-opus-4-6-system-card] No AIBOM can close the visibility gap created by model inscrutability and limited reasoning-faithfulness assessment, so it cannot supply complete explainability or guarantee safe behavior.
- [inference; source: https://www.nist.gov/itl/ai-risk-management-framework; https://davidamitchell.github.io/Research/research/2026-05-06-aibom-platform-observability-control-comparison.html] The right success test is therefore not "Is the AIBOM complete?" but "Did AIBOM-informed controls improve structural coverage, shorten drift detection, reduce reachable blast radius, and support better incident reconstruction?"
...
6. [inference; source: https://www.nist.gov/itl/ai-risk-management-framework; https://www.anthropic.com/claude-opus-4-6-system-card] Inference opacity is an irreducible limit on AIBOM effectiveness, because current governance and interpretability practice can document inputs, outputs, and conditions of use but cannot fully expose or validate internal reasoning.
...
| [inference] Inference opacity is an irreducible AIBOM boundary. | https://www.nist.gov/itl/ai-risk-management-framework ; https://www.anthropic.com/claude-opus-4-6-system-card | medium | reasoning remains partly opaque |
...
- [inference; source: https://www.ntia.gov/page/software-bill-materials; https://www.nist.gov/itl/ai-risk-management-framework] The evidence was weighted by first asking which SBOM-style benefits clearly transfer to artificial intelligence systems and then testing whether the relevant object was an inventoryable structure, a runtime event, or a semantic behavior.
...
- [fact; source: https://www.nist.gov/itl/ai-risk-management-framework] NIST says robust and verifiable risk-measurement methods remain an open challenge, so the metric formulas proposed here should be treated as operational starting points rather than settled industry standards.
...
- Unresolved uncertainty: empirical AIBOM deployment outcome data remains limited
...
An AIBOM creates false assurance when its completeness is mistaken for semantic safety, because prompt injection, Retrieval-Augmented Generation (RAG) poisoning, memory poisoning, and legitimate-tool misuse can all succeed through correctly declared channels. [inference; source: https://arxiv.org/abs/2302.12173; https://arxiv.org/abs/2211.09527; https://www.microsoft.com/en-us/security/blog/2025/04/24/new-whitepaper-outlines-the-taxonomy-of-failure-modes-in-ai-agents/]

No AIBOM can close the visibility gap created by model inscrutability and limited reasoning-faithfulness assessment, so it cannot supply complete explainability or guarantee safe behavior. [inference; source: https://www.nist.gov/itl/ai-risk-management-framework; https://www.anthropic.com/claude-opus-4-6-system-card]

The right success test is therefore not whether the AIBOM is complete, but whether AIBOM-informed controls improve structural coverage, shorten drift detection, reduce reachable blast radius, and support better incident reconstruction. [inference; source: https://www.nist.gov/itl/ai-risk-management-framework; https://davidamitchell.github.io/Research/research/2026-05-06-aibom-platform-observability-control-comparison.html]
...
Robust and verifiable risk-measurement methods remain an open challenge, so the metric formulas proposed here should be treated as operational starting points rather than settled industry standards. [fact; source: https://www.nist.gov/itl/ai-risk-management-framework]
