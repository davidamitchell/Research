...
- [fact] Solar-Lezama's lecture notes define Hoare triples as partial correctness assertions that guarantee the postcondition only if the program terminates, while total correctness additionally requires termination. [source: https://people.csail.mit.edu/asolar/SynthesisCourse/Lecture18.htm]
- [fact] The same notes state that weakest preconditions can be computed algorithmically for loop-free programs, but not in general for programs with loops, and that loop proofs require invariants and ranking functions supplied with extra structure. [source: https://people.csail.mit.edu/asolar/SynthesisCourse/Lecture18.htm]
- [fact] Schultz's model-checking notes describe model checking as verifying whether a transition system satisfies a temporal-logic specification and state that the basic setting assumes a finite-state model. [source: https://will62794.github.io/my-notes/notes/Model_Checking/Model_Checking.html]
- [fact] The same notes state that there is a linear-time algorithm for Computation Tree Logic (CTL) model checking in the size of the finite Kripke structure and the formula, and an algorithm for Linear Temporal Logic (LTL) model checking that is linear in the structure size and exponential in formula length. [source: https://will62794.github.io/my-notes/notes/Model_Checking/Model_Checking.html]
...
4. Abstract interpretation exists precisely because useful analysis remains possible only through abstraction that preserves soundness while tolerating inaccuracy and incomplete results about concrete executions. ([fact]; high confidence; source: https://www.di.ens.fr/~cousot/COUSOTpapers/POPL77.shtml; https://www.di.ens.fr/~cousot/AI/)
...
| [fact] Abstract interpretation makes analysis workable through sound but inexact abstraction over program semantics. | https://www.di.ens.fr/~cousot/COUSOTpapers/POPL77.shtml ; https://www.di.ens.fr/~cousot/AI/ | high | approximation trade-off |
...
| [fact] Model checking is algorithmic on finite-state transition systems and bounded temporal-logic problems. | https://will62794.github.io/my-notes/notes/Model_Checking/Model_Checking.html | medium | decidability under restriction |
...
- The item does not survey decidable subclasses such as total functional languages, proof assistants, or specialised termination provers, so it should not be read as claiming that all useful verification is impossible. [fact; source: https://will62794.github.io/my-notes/notes/Model_Checking/Model_Checking.html; https://people.csail.mit.edu/asolar/SynthesisCourse/Lecture18.htm; https://www.di.ens.fr/~cousot/AI/]
...
- The item deliberately omits a catalogue of decidable subclasses, so it establishes the universal boundary more clearly than it maps the full menu of tractable special cases. [fact; source: https://will62794.github.io/my-notes/notes/Model_Checking/Model_Checking.html; https://people.csail.mit.edu/asolar/SynthesisCourse/Lecture18.htm; https://www.di.ens.fr/~cousot/AI/]
