...
- [x] Princeton Language and Intelligence, HAL Reliability Dashboard - https://hal.cs.princeton.edu/reliability/
...
- [fact] Research question restated: the task is to determine how an AI finisher can reliably convert a human's partial ideas into completed, structured, reviewable work without adding a new layer of half-finished output. Source: item brief in this file.
- [fact] Scope confirmed: the investigation covers cognitive complementarity, agent roles, handoff and prompt patterns, finishing-specific failure modes, and tool fit under a browser-first GitHub workflow. Source: item brief in this file.
- [fact] Constraint confirmed: the owner operates through GitHub website and iOS app only, so the recommended pattern must work without a local IDE or unapproved credentials. Source: item brief in this file; `.github/copilot-instructions.md`.
- [fact] Prior-work cross-reference completed before external investigation. The most relevant completed items are the superpowers adaptation item, the explore-to-exploit synthesis item, the coding agent skills survey, and the applied context engineering item. Sources: `Research/completed/2026-03-12-superpowers-integration-analysis.md`; `Research/completed/2026-03-18-explore-to-exploit-synthesis-gap.md`; `Research/completed/2026-03-22-coding-ai-agent-skills-survey.md`; `Research/completed/2026-03-22-applied-context-engineering-agent-workflows.md`.
- [fact] Skill-process fallback applied: `.github/skills/research/SKILL.md` is not present in this checkout, and the repo instructions say to follow `research-prompt.md` when the submodule is not initialised. Source: `.github/copilot-instructions.md`; local repository inspection.
- [fact] Output format confirmed: write a full research record in §§0-§7, then seed `## Findings` directly from §6 without introducing new claims. Source: item brief in this file.
...
- [fact] This repository already uses persistent instruction artifacts and workflow state machines: `.github/copilot-instructions.md` encodes durable repo norms, and `.github/workflows/research-loop.yml` enforces start/draft/complete boundaries, caps iterations, and pushes through GitHub-native automation. Sources: `.github/copilot-instructions.md`; `.github/workflows/research-loop.yml`
...
- [assumption] If the owner later wants richer multi-agent orchestration beyond GitHub-native flows, that work would require additional runtime ownership and possibly capability confirmations that are outside the currently documented credential set. Justification: `.github/copilot-instructions.md` prohibits assuming undocumented services or credentials.

#### 2.5 Failure modes, controls, and practical limits

Sources consulted:
- Primary: HAL Reliability Dashboard - https://hal.cs.princeton.edu/reliability/
- Primary: NIST AI RMF - https://doi.org/10.6028/NIST.AI.100-1
- Primary: Anthropic, "Building effective agents" - https://www.anthropic.com/research/building-effective-agents
- Primary: GitHub Copilot coding agent docs - https://docs.github.com/en/copilot/concepts/agents/coding-agent/about-coding-agent

Claims:
- [fact] The HAL Reliability Dashboard finds that reliability has improved only modestly relative to capability gains, and that consistency, predictability, and prompt robustness remain distinct problems even for strong frontier agents. Source: https://hal.cs.princeton.edu/reliability/
- [fact] HAL specifically recommends multi-run evaluation, multi-condition perturbation, and deployment governance that distinguishes augmentation use cases from automation use cases. Source: https://hal.cs.princeton.edu/reliability/
...
Claims:
- [fact] This repository already contains the primitives needed for a finisher pattern: durable instructions, GitHub-native automation, explicit state transitions, and review workflows. Sources: `.github/copilot-instructions.md`; `.github/workflows/research-loop.yml`
...
- [fact] No contradiction was found between the cognition literature and the tooling literature. The cognition sources recommend external structure to reduce executive load, and the tooling sources identify durable instructions, workflow gates, and explicit artifacts as the mechanisms that provide that structure. Sources: Risko and Gilbert 2016; Berry et al. 2019; GitHub docs; Anthropic workflow guidance.
- [inference] The only meaningful tension is between agent flexibility and reliability: richer autonomy can cover more cases, but the available reliability evidence shows that consistency and predictability still lag raw capability. Basis: Anthropic guidance and HAL results.
...
- [fact] The repository's own evolution already trends toward durable operating artifacts such as instructions, skills, and workflows rather than ad hoc prompting, which is consistent with the evidence gathered here. Sources: `.github/copilot-instructions.md`; `.github/workflows/research-loop.yml`; prior completed items reviewed in §0.
- [inference] From a governance lens, the complementary model stays credible only while the human continues to own intent, acceptance, and exception handling. If those are delegated too early, the failure modes shift from incomplete work to confidently wrong work. Basis: HAL reliability guidance; NIST AI RMF; GitHub review workflow.
...
5. [inference][medium] For the owner's no-local-IDE workflow, adopting GitHub-native instructions, review loops, and workflow gates will yield more immediate finishing value than introducing a bespoke multi-agent framework. Sources: https://docs.github.com/en/copilot/how-tos/configure-custom-instructions/add-repository-instructions; https://github.blog/ai-and-ml/github-copilot/github-copilot-coding-agent-101-getting-started-with-agentic-workflows-on-github/; repository workflow and instructions files.
...
8. [fact][medium] GitHub officially supports incremental behaviour shaping through repository custom instructions, agent instruction files, skills, hooks, and custom agents, which means finisher behaviour can be improved through additive repository artifacts rather than a full platform migration. Sources: https://docs.github.com/en/copilot/how-tos/configure-custom-instructions/add-repository-instructions; https://docs.github.com/en/copilot/concepts/agents/coding-agent/about-coding-agent

**Evidence map:**

| Claim | Source | Confidence | Notes |
|---|---|---|---|
| Bounded planner-executor-synthesiser-reviewer workflow beats unconstrained autonomy for finishing tasks. | Anthropic workflow taxonomy; GitHub coding agent docs | high | Supported by first-party workflow guidance and GitHub review model. |
| Complementarity is strongest when humans stay on divergent work and agents take convergent work. | Leidner et al. 2025; Design Council Double Diamond; Wigert et al. 2024 | high | Convergent/divergent split plus formal complementarity framing. |
| Persistent handoff artifacts reduce intent loss and executive burden. | Risko and Gilbert 2016; Berry et al. 2019; GitHub custom instructions docs | high | Cognitive offloading plus durable repository instructions. |
| GitHub Copilot coding agent fits browser-only finishing better than CrewAI, AutoGen, or LangGraph. | GitHub coding agent docs; CrewAI docs; AutoGen docs; LangGraph docs | high | Direct capability and operating-model comparison. |
| Repository instructions, handoff contracts, and reviewer gates should be improved before bespoke multi-agent orchestration is introduced. | GitHub custom instructions docs; GitHub blog; Anthropic effective agents | medium | Strong fit inference from first-party docs and workflow guidance. |
| Reliability limits still block full autonomy for ambiguous work. | HAL Reliability Dashboard; Anthropic effective agents | high | Reliability dimensions lag raw capability. |
| Completion control should rely on explicit done criteria, validation, review, and escalation. | HAL Reliability Dashboard; NIST AI RMF; GitHub coding agent docs | high | Governance and deployment controls align across sources. |
| Finisher behaviour can be improved through additive repository artifacts such as instructions, skills, hooks, and custom agents. | GitHub custom instructions docs; GitHub coding agent docs | medium | GitHub documents several repo-level control surfaces for behaviour shaping. |

**Assumptions:**

- [assumption] The initial deployment target is bounded repository or document work that can be expressed as an issue, task brief, or section-level deliverable rather than as open-ended personal decision-making. **Justification:** GitHub coding agent and the HAL guidance are both strongest for bounded, reviewable tasks. Sources: https://docs.github.com/en/copilot/concepts/agents/coding-agent/about-coding-agent; https://hal.cs.princeton.edu/reliability/
...
- [assumption] A good outcome means higher completion throughput with bounded risk, not maximum agent autonomy. **Justification:** Anthropic and the HAL Reliability Dashboard both treat reliability and controllability as first-order deployment criteria, not optional extras. Sources: https://www.anthropic.com/research/building-effective-agents; https://hal.cs.princeton.edu/reliability/
...
- [fact] Every section from §0 through §6 has been populated, and each claim in the investigation, reasoning, consistency, depth-and-breadth, and synthesis sections is explicitly labeled as [fact], [inference], or [assumption]. Source: this document.
- [fact] The synthesis is consistent with the investigation: the Findings recommend GitHub-native bounded finishing, durable handoff artifacts, and human review because those are the positions supported repeatedly across the cognition, tooling, and reliability sources. Source: sections 2.1-2.6 above.
- [fact] Acronym expansion audit completed for the terms introduced in the Research Skill Output and Findings, including Artificial Intelligence (AI), Application Programming Interface (API), Large Language Model (LLM), Integrated Development Environment (IDE), Command Line Interface (CLI), pull request (PR), and Artificial Intelligence Risk Management Framework (AI RMF). Source: this document.
- [inference] The item is ready to seed Findings and proceed to draft review because the remaining uncertainty is about implementation priority, not about the main conclusion. Basis: completed evidence map and identified open questions.
...
- [inference] Artificial Intelligence (AI) agents can act as reliable finishers and synthesisers for ideation-strong, execution-weak humans when they are used inside a bounded GitHub-native workflow that preserves human ownership of intent and final acceptance while assigning the agent explicit planning, execution, validation, and packaging work. Sources: https://docs.github.com/en/copilot/concepts/agents/coding-agent/about-coding-agent; https://www.anthropic.com/research/building-effective-agents; https://hal.cs.princeton.edu/reliability/
...
- [inference] For browser-first GitHub operating models, GitHub Copilot coding agent plus repository instructions, workflow gates, and review loops are a stronger fit than CrewAI, AutoGen, or LangGraph as immediate finishing tools, because the latter assume code-level orchestration and owned runtime surfaces. Sources: https://docs.github.com/en/copilot/concepts/agents/coding-agent/about-coding-agent; https://docs.crewai.com/concepts/agents; https://microsoft.github.io/autogen/stable/; https://docs.langchain.com/oss/python/langgraph/overview
...
| Claim | Source | Confidence | Notes |
|---|---|---|---|
| Explicit planner-executor-synthesiser-reviewer loops improve finishing reliability. | Anthropic workflow taxonomy; GitHub coding agent docs | high | Supported by first-party workflow patterns and GitHub review surface. |
| Complementarity works best when humans stay on divergent work and agents take convergent work. | Leidner et al. 2025; Design Council; Wigert et al. 2024 | high | Theory and process evidence align. |
| Durable handoff artifacts reduce intent loss and executive burden. | Risko and Gilbert 2016; Berry et al. 2019; GitHub instructions docs | high | Cognitive offloading plus repository-instruction mechanism. |
| GitHub Copilot coding agent fits browser-only finishing better than CrewAI, AutoGen, or LangGraph. | GitHub docs; CrewAI docs; AutoGen docs; LangGraph docs | high | Tooling capabilities and operating assumptions differ clearly. |
| Repository instructions, handoff contracts, and reviewer gates should be improved before bespoke multi-agent orchestration is introduced. | GitHub docs; GitHub blog; Anthropic effective agents | medium | Strong fit inference from first-party docs and workflow guidance. |
| Reliability gaps still prevent unchecked autonomous finishing for ambiguous work. | HAL Reliability Dashboard; Anthropic effective agents | high | Consistency and prompt robustness remain unresolved. |
| Bounded scope, validation, review, and escalation are the right completion controls. | HAL Reliability Dashboard; NIST AI RMF; GitHub coding agent docs | high | Governance and workflow controls converge. |
| Finisher behaviour can be improved through additive repository artifacts such as instructions, skills, hooks, and custom agents. | GitHub custom instructions docs; GitHub coding agent docs | medium | GitHub documents several repo-level control surfaces for behaviour shaping. |
...
- [inference] The public evidence base is stronger for coding and structured completion than for personal productivity or personal knowledge management finishing, so these conclusions are strongest for repository workflows. Basis: source distribution gathered in this item.
- [inference] The complementarity literature is newer and less extensive than the tooling literature, so the psychological explanation should be treated as supportive framing rather than closed theory. Basis: source set reviewed in this item.
- [inference] The exact threshold at which this repo should graduate from GitHub-native finishing to custom multi-agent orchestration remains uncertain and should be decided empirically after a simpler finisher contract is piloted. Basis: overall evidence weighting.
