...
Platform engineering (the discipline of building self-service internal delivery platforms), InnerSource (the use of open-source collaboration principles inside an organisation), and standard-core plus local-extension models each address a distinct layer of the fragmentation-versus-standardisation problem, and the most reliable anti-fragmentation designs combine all three rather than treating any one as sufficient. The golden path pattern, self-service with guardrails, and voluntary adoption are the most consistently documented patterns for preserving local agility: they set an opinionated default while maintaining a sanctioned extension path, which removes the incentive for teams to fork local copies of shared assets. Mandatory centralisation without extension points produces a bypass response equivalent to the multidomestic archetype in Bartlett and Ghoshal's transnational framework, recreating the fragmentation the platform was built to prevent. The DORA 2024 report provides a concrete caution: platform engineering improves productivity but can decrease change stability when platform components create shared single-point dependencies, meaning extension points serve not only local agility but also resilience. [inference; source: https://engineering.atspotify.com/2020/08/how-we-use-golden-paths-to-solve-fragmentation-in-our-software-ecosystem; https://dora.dev/research/2024/dora-report/; https://hbsp.harvard.edu/product/9498-PDF-ENG; https://patterns.innersourcecommons.org/p/trusted-committer]
...
- Quantitative evidence on InnerSource adoption rates, fragmentation reduction percentages, or Trusted Committer network scaling limits is absent from the reviewed literature; all InnerSource pattern evidence is case-study based with named instances rather than large-scale empirical studies.
- The Bartlett and Ghoshal (1989) primary source is a book-length academic treatment not freely accessible online; the secondary-source descriptions of the four archetypes are consistent across multiple independent sources but the specific causal mechanisms they describe may not transfer exactly to software tooling governance contexts.
...
acronym_audit:
  ...
  - IDP: expanded as "Internal Developer Platform (IDP)" in §2 A3
  ...
  - CI/CD: not used in claim-bearing prose; only appears as an allusion in cited source
    descriptions; no expansion required
...
findings_inline_labels: all Executive Summary and Analysis sentences carry trailing
                        ...
                        ([fact/inference]; confidence; source: URL) format
