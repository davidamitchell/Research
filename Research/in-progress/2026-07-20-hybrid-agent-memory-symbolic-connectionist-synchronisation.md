...
cross_item_alignment: consistent with tbox-abox, truth-maintenance, consolidation, mcp-schema items
scope_guardrail: maintained, no ontology-authoring or pure-vector-RAG claims introduced
acronym_audit: LLM, RAG, KG, MCP, DMR, BM25, W3C, RDF, p95 expanded on first use
```
...
**Executive summary:** Production hybrid agent-memory systems keep the symbolic knowledge graph and the unstructured retrieval layer synchronised by making incremental, event-triggered writes into a single non-lossy store, invalidating superseded facts with bi-temporal timestamps rather than deleting or rebuilding them, and indexing the retrieval layer directly over that same store so the two views cannot drift apart. Full graph rebuilds are reserved for schema or model changes because their cost scales with corpus size. Contradiction handling is a purpose-built detect-then-resolve or recency-priority stage, not a formal truth-maintenance system.

**Key findings:** see numbered list in Findings.

**Evidence map:** see table in Findings.

**Assumptions:** vendor-authored benchmark numbers are treated as directional evidence, not independently replicated fact; the four prior completed items are treated as settled internal baselines.

**Analysis:** Zep's invalidate-with-timestamp design and Mem0's accumulate-and-rank design are two routes to the same goal of never destructively overwriting history; GraphRAG's incremental methods show the cost boundary that forces incremental over full rebuild.

**Risks, gaps, uncertainties:** benchmarks are self-reported; no located source measures long-run drift directly; human-in-the-loop graph gating is under-documented in these systems.

**Open questions:** how to measure symbolic-connectionist drift directly; whether ADD-only or invalidate-with-timestamp scales better past tens of millions of facts.
...
claim_label_audit: passed, every §2 to §5 claim carries fact, inference, or assumption
...
| [inference] No production TMS; purpose-built resolution substitutes | https://davidamitchell.github.io/Research/research/2026-07-20-autonomous-knowledge-curation-truth-maintenance.html; https://arxiv.org/abs/2501.13956; https://arxiv.org/abs/2504.19413 | medium | prior item plus two primaries |
