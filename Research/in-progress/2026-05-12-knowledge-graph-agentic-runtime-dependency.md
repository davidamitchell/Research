...
**In scope:**
- Architectural patterns for coupling an agent pipeline to a KG at runtime, including synchronous query, recently refreshed caching, and pre-generated context injection
...
- [inference] Working definition for this item: treat the graph as a live serving dependency whenever the system's next answer or tool action depends on a graph query, a graph-backed cache, or a derived graph summary whose freshness window must be managed operationally rather than assumed. [source: https://arxiv.org/abs/2404.16130; https://arxiv.org/abs/2312.10997; https://martinfowler.com/bliki/CircuitBreaker.html]
...
- [inference] These sources imply three recurrent runtime coupling patterns: direct synchronous graph queries for the current step, caches or subgraph snapshots with explicit freshness limits for repeated local reads, and pre-generated graph summaries that trade freshness for lower online query cost. [source: https://arxiv.org/abs/2404.16130; https://www.w3.org/TR/sparql11-query/; https://raw.githubusercontent.com/microsoft/graphrag/main/docs/index/outputs.md]
...
- [inference] A Knowledge Graph that serves both writes and immediate follow-up reads therefore needs an explicit consistency contract per agent step: use causal reads for write-dependent actions, and permit cached or replica reads only when the accepted lag is appropriate for advisory context. [source: https://neo4j.com/docs/query-api/current/bookmarks/; https://neo4j.com/docs/python-manual/current/bookmarks/; https://docs.aws.amazon.com/neptune/latest/userguide/cw-metrics.html]
...
- [inference] The reliable design pattern is a tiered dependency: an authoritative live graph for strict reads, a cache or snapshot with an explicit freshness limit for low-risk context, and explicit failure handling that narrows scope, serves stale context, or defers the action instead of masking graph failure. [source: https://martinfowler.com/bliki/CircuitBreaker.html; https://arxiv.org/abs/2404.16130; https://neo4j.com/docs/query-api/current/bookmarks/; https://docs.aws.amazon.com/neptune/latest/userguide/cw-metrics.html]
...
- [assumption] Most teams can tolerate recently refreshed graph context for synthesis or retrieval ranking, but not for steps that create, approve, or validate stateful actions. [source: https://martinfowler.com/bliki/CircuitBreaker.html; https://neo4j.com/docs/query-api/current/bookmarks/; https://docs.aws.amazon.com/neptune/latest/userguide/cw-metrics.html]
