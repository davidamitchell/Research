...
- [fact] **Research question restated:** the task is to determine how an organisation that already runs GHAS and CodeQL can extend GitHub Actions into a shared compliance platform for naming, structure, architecture, API and event specifications, and stack-specific engineering rules across heterogeneous repositories. Source: item definition above.
- [fact] **Scope confirmation:** the question is about pre-merge and scheduled compliance enforcement in GitHub Actions, not runtime control enforcement, identity policy, or a greenfield GHAS installation. Source: item scope above.
- [fact] **Output format:** the required deliverable is a fully sourced knowledge item with claim labels, a stack map, a minimum viable architecture, trade-offs, and open questions that can become backlog items. Source: research-loop prompt and item output definition.
- [fact] **Prior work cross-reference:** `2026-03-16-adaptive-policy-authorization-compliance.md` established that PaC engines such as OPA, Cedar, and Cerbos are valuable when they produce durable review, validation, and audit artefacts, and `2026-03-01-agent-lsp-policy-enforcement.md` established that policy feedback loops usually need a custom bridge when the native tool is not agent-aware. Sources: `Research/completed/2026-03-16-adaptive-policy-authorization-compliance.md`; `Research/completed/2026-03-01-agent-lsp-policy-enforcement.md`.
...
- [inference] Checkov is the broadest single IaC scanner in the retrieved evidence because it spans Terraform, Kubernetes, Ansible, OpenAPI, and GitHub Actions, while kube-score adds Kubernetes-specific resilience and workload-quality checks that Checkov alone does not express with the same operational focus. Basis: Checkov and kube-score sources above.
- [inference] The AWS pattern is direct evidence that "define policies once, enforce everywhere" is already practical in GitHub Actions using a central policy repository and reusable workflows, even before adding GitHub organisation rulesets on top. Basis: AWS Prescriptive Guidance above.
...
- [inference] DBT and Snowflake compliance need dedicated SQL- and dbt-aware tooling because CodeQL does not natively model dbt project structure or SQL dialect/style semantics, while SQLFluff plus dbt-project-evaluator directly target those domains. Basis: SQLFluff and dbt-project-evaluator documentation plus CodeQL scope above.
...
- [inference] Go services benefit from golangci-lint for broad idiomatic and quality checks, while Semgrep or CodeQL can be reserved for organisation-specific or security-focused Go policies. Basis: golangci-lint, Semgrep, and CodeQL documentation above.
...
- [inference] **Economic lens:** the open-source stack in this item is already sufficient for a credible minimum viable compliance platform, which means the primary cost is policy authorship, tuning, and maintenance rather than tool licensing. Basis: GitHub-native primitives plus open-source scanners above.
- [inference] **Developer-behaviour lens:** false positives and duplicated scanner output are likely to be the biggest adoption risk, because broad policy portfolios create more ways to fail a build and require consistent waiver, severity, and ownership models across scanners. Basis: multi-tool architecture above and GitHub guidance on staged rollout.
...
- [inference] The exact commercial-plan and feature-state history for "required workflows" is confusing across older GitHub blog posts and newer rulesets documentation, so implementation should be anchored to current docs in the target GitHub plan rather than older launch posts. Basis: GitHub blog update text versus current rulesets docs.
- [inference] GraphQL governance is only partially covered here, because the retrieved evidence confirms GraphQL Inspector for schema change and document validation but does not evaluate the wider GraphQL style-guide ecosystem. Basis: GraphQL Inspector docs.
...
- [fact] GHAS and CodeQL can support broad compliance scanning across a heterogeneous GitHub estate only when they are used as the supported-language security baseline inside a larger GitHub Actions architecture that adds reusable workflows, rulesets, and specialised scanners for manifests, schemas, SQL, and project structure. Sources: https://docs.github.com/en/code-security/code-scanning/introduction-to-code-scanning/about-code-scanning-with-codeql ; https://docs.github.com/en/actions/sharing-automations/reusing-workflows ; https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/managing-rulesets/available-rules-for-rulesets#require-workflows-to-pass-before-merging
- [fact] GitHub already provides the native cross-repository enforcement primitives needed for this design: reusable workflows centralise execution logic, while organisation rulesets and required status checks make shared checks mandatory. Sources: https://docs.github.com/en/actions/sharing-automations/reusing-workflows ; https://docs.github.com/en/enterprise-cloud@latest/organizations/managing-organization-settings/creating-rulesets-for-repositories-in-your-organization
...
1. [fact][high] GHAS and CodeQL cover the supported-language part of the estate well, but the official GitHub and CodeQL documentation does not extend that coverage to Terraform policy, Kubernetes workload quality, OpenAPI or AsyncAPI governance, SQL dialect rules, or dbt project structure. Sources: https://docs.github.com/en/code-security/code-scanning/introduction-to-code-scanning/about-code-scanning-with-codeql ; https://codeql.github.com/docs/codeql-overview/supported-languages-and-frameworks/
2. [fact][high] GitHub reusable workflows and organisation rulesets already provide the central execution and enforcement mechanism needed to define compliance checks once, share them broadly, and block merges when required checks do not pass. Sources: https://docs.github.com/en/actions/sharing-automations/reusing-workflows ; https://docs.github.com/en/enterprise-cloud@latest/organizations/managing-organization-settings/creating-rulesets-for-repositories-in-your-organization ; https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/managing-rulesets/available-rules-for-rulesets#require-workflows-to-pass-before-merging
...
| Claim | Source | Confidence | Notes |
|---|---|---|---|
| [fact] GHAS and CodeQL are a baseline, not a full heterogeneous compliance platform. | https://docs.github.com/en/code-security/code-scanning/introduction-to-code-scanning/about-code-scanning-with-codeql ; https://codeql.github.com/docs/codeql-overview/supported-languages-and-frameworks/ | high | Source-backed language list makes the coverage boundary visible. |
| [fact] Reusable workflows plus rulesets provide central execution and enforcement. | https://docs.github.com/en/actions/sharing-automations/reusing-workflows ; https://docs.github.com/en/enterprise-cloud@latest/organizations/managing-organization-settings/creating-rulesets-for-repositories-in-your-organization ; https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/managing-rulesets/available-rules-for-rulesets#require-workflows-to-pass-before-merging | high | This is the GitHub-native core of the recommended architecture. |
