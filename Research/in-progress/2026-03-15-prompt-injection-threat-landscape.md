...
1. **Severity has escalated with agency.** In 2022-2023 prompt injection was primarily a nuisance (jailbreaks, data exfiltration via the model's own output). **[inference]** In 2024-2025, agentic systems that browse, write code, call APIs, and send messages mean a successful injection can result in account takeover, data destruction, or lateral movement in the attacker's supply chain, so the threat model has materially changed. (Sources: https://genai.owasp.org/llmrisk/llm01-prompt-injection/ ; https://arxiv.org/abs/2302.12173 ; https://unit42.paloaltonetworks.com/ai-agent-prompt-injection/)
...
3. **Standards work is moving quickly.** OWASP LLM01:2025, NIST AI 100-2, and updated vendor guidance from Anthropic, Google DeepMind, Microsoft, and security companies all moved during 2024-2025, so a dedicated current-state synthesis is warranted.
...
**Research question restated:** What is the current state of the prompt injection threat in agentic AI systems, specifically: which attack types matter most, who is credibly exploiting them, who is building defences, and what remains unsolved in the 2024-2025 research front?

**Scope confirmed:** In scope are direct prompt injection, indirect prompt injection, compositional and multi-turn prompt injection, multimodal prompt injection where it materially changes the threat surface, documented incidents and CVEs, standards and frameworks, vendor defences, and the design implications for agents with tools, memory, and external side effects. Out of scope are training-data poisoning, non-LLM adversarial machine-learning attacks, and broad AI-governance questions not specific to prompt injection.
...
**Source classes used:** Primary - OWASP LLM01, NIST AI 100-2, Greshake et al. (2023). Secondary - Simon Willison's prompt injection series.

**[fact]** OWASP LLM01:2025 defines prompt injection as a vulnerability in which prompts alter model behaviour or output in unintended ways, and it distinguishes **direct prompt injections** from **indirect prompt injections** where the model consumes attacker-controlled external content such as documents or web pages. (Sources: OWASP GenAI LLM01 page; OWASP LLM project overview)

**[fact]** OWASP states that the impact of prompt injection depends heavily on **agency**: a successful injection can disclose sensitive information, reveal system prompts or infrastructure details, gain unauthorised access to functions, execute arbitrary commands in connected systems, or manipulate critical decision-making. (Source: OWASP GenAI LLM01 page)

**[fact]** Greshake et al. argue that Large Language Model-integrated applications blur the boundary between **data** and **instructions**, enabling **indirect prompt injection** in which attackers remotely exploit systems by placing malicious instructions into data likely to be retrieved later by the application. They demonstrate data theft, worming, information-ecosystem contamination, arbitrary code-execution-like effects, and manipulated Application Programming Interface calls. (Source: arXiv:2302.12173)

**[fact]** NIST AI 100-2 is a taxonomy of adversarial machine-learning attacks and mitigations, and the publication positions misuse and abuse of model interfaces as a recognised security class alongside evasion, poisoning, and privacy attacks. (Source: NIST AI 100-2 PDF)

**[fact]** OWASP explicitly adds multimodal prompt injection scenarios, including hidden instructions in images, and notes that current multimodal-specific defences remain immature. (Source: OWASP GenAI LLM01 page)
...
**[fact]** Greshake et al. demonstrated prompt injection against real systems including Bing's GPT-4-powered chat and code-completion engines in 2023, showing that indirect prompt injection is not restricted to toy examples. (Source: arXiv:2302.12173)

**[fact]** OWASP's LLM01 reference scenarios cite public plugin and document-based prompt injection cases, including document poisoning, cross-plugin request forgery, resume injection, payload splitting, code injection, multilingual and obfuscated attacks, and multimodal attacks. (Source: OWASP GenAI LLM01 page)

**[fact]** Simon Willison's long-running series documents the evolution from early chatbot jailbreaks to prompt injection against browsing systems, retrieval pipelines, audio models, Model Context Protocol (MCP) tooling, and agent architectures. His 2025 writing repeatedly argues that prompt injection remains unsolved and that the key mitigation is to **limit blast radius**, not to assume the model can perfectly distinguish instructions from data. (Source: Simon Willison prompt injection series)

**[fact]** Palo Alto Networks Unit 42 reports that web-based indirect prompt injection is being **actively weaponized**. Their telemetry analysis claims 22 distinct payload-engineering techniques in the wild and documents attacker intents including ad-review evasion, search-engine-optimization manipulation, data destruction, denial of service, unauthorised transactions, sensitive information leakage, and system-prompt leakage. (Source: Unit 42 web-based IDPI report)

**[fact]** Unit 42 also states that prior work focused largely on proof-of-concept risk, whereas its own telemetry shows web-based indirect prompt injection moving beyond theory into observed malicious deployment. (Source: Unit 42 web-based IDPI report)

**[fact]** OWASP includes a code-injection scenario referencing CVE-2024-5184, showing that the standards community now treats prompt injection as a route to concrete downstream exploitation rather than merely a content-safety issue. (Source: OWASP GenAI LLM01 page)
...
**Source classes used:** Primary - Anthropic, Microsoft, Google DeepMind, CaMeL paper, Lakera benchmark repository, Adaptive Attacks paper. Secondary - Simon Willison's analysis where it clarifies architectural trade-offs.

**[fact]** Anthropic's many-shot jailbreaking research shows that longer context windows create a new attack surface: increasing the number of malicious demonstrations in-context increases jailbreak effectiveness, and simple fine-tuning only delays rather than removes the failure. Anthropic reports one prompt-based mitigation reducing attack success from 61% to 2% in one setting. (Source: Anthropic many-shot jailbreaking)

**[fact]** Anthropic's Constitutional Classifiers use input and output classifiers trained on synthetic constitutional data. In Anthropic's automated evaluation of 10,000 advanced jailbreak prompts, jailbreak success fell from 86% on the base model to 4.4% with classifiers, with a reported 0.38% increase in refusals and 23.7% higher compute cost. (Source: Anthropic Constitutional Classifiers)

**[fact]** Anthropic's own limitations section says Constitutional Classifiers may not prevent every universal jailbreak and recommends complementary defences. Their February 2025 live demo update reported that one universal jailbreak was eventually found after sustained community red teaming. (Source: Anthropic Constitutional Classifiers)

**[fact]** Microsoft Prompt Shields separates **user prompt attacks** from **document attacks** and recognises manipulated content, information gathering, fraud, malware, encoded attacks, role-play, conversation mockups, and attempts to change system rules. It is explicitly positioned as a specialised filter that sits around the model rather than as a proof that the model itself is robust. (Source: Microsoft Prompt Shields documentation)

**[fact]** Lakera's public Prompt Injection Test (PINT) Benchmark repository reports 4,314 test inputs spanning English and non-English inputs. Its May 2025 published scores show substantial variance across detectors: Lakera Guard 95.2200%, Amazon Web Services Bedrock Guardrails 89.2404%, Azure AI Prompt Shield Documents + User Prompts 89.1241%, and several smaller models far lower. (Source: Lakera PINT GitHub repository)

**[fact]** Google DeepMind's capability-based dual-model defence architecture proposes separating trusted control flow from untrusted data flow using a privileged model, a quarantined model, and capability enforcement around tool calls. The paper reports 77% secure task completion with provable security in AgentDojo, compared with 84% task completion for an undefended system. (Source: arXiv:2503.18813)

**[fact]** Google DeepMind's Gemini security paper frames indirect prompt injection as a live issue for tool-using models that access user data, and it emphasises continuous **adaptive adversarial evaluation** against past, current, and future model versions rather than one-off testing. (Source: "Lessons from Defending Gemini Against Indirect Prompt Injections" PDF)

**[fact]** Zhan et al. evaluate eight defences against indirect prompt injection on LLM agents and report bypassing all eight with adaptive attacks, with attack success rates consistently above 50%. (Source: Findings of the Association for Computational Linguistics: North American Chapter of the Association for Computational Linguistics 2025 paper)
...
**Source classes used:** Primary - Anthropic many-shot paper, Anthropic Constitutional Classifiers, CaMeL paper, Adaptive Attacks paper. Secondary - Simon Willison series for research-front synthesis.

**[fact]** The most consequential 2024-2025 contributions in this research slice are:
1. **Many-shot jailbreaking** (Anthropic, 2024) - shows that long context windows themselves amplify jailbreak vulnerability.
2. **Constitutional Classifiers** (Anthropic, 2025) - provides one of the clearest published metrics for meaningful but incomplete jailbreak reduction.
3. **Defeating Prompt Injections by Design / CaMeL** (Google DeepMind, 2025) - shifts the field from prompt filtering toward capability-based architectural isolation.
4. **Adaptive Attacks Break Defenses Against Indirect Prompt Injection Attacks on LLM Agents** (Zhan et al., 2025) - demonstrates that state-of-the-art defences fail under adaptive evaluation.
5. **Google DeepMind's Gemini security paper** (2025) - operationalises continuous adversarial evaluation for indirect prompt injection in deployed systems.
6. **Lakera's PINT Benchmark** (2024-2025 updates) - gives the ecosystem a comparative detector benchmark instead of vendor-specific anecdotes.

**[fact]** Simon Willison's 2025 writing on CaMeL and agent design makes the same structural point from the practitioner side: powerful general-purpose agents with private data, untrusted content, and external communications create a "lethal trifecta" for prompt injection risk. (Source: Simon Willison prompt injection series)
...
**Source classes used:** Primary - OWASP LLM01, Microsoft Prompt Shields, Anthropic and Google DeepMind defence papers. Secondary - Simon Willison's blast-radius framing.

**[fact]** OWASP recommends constraining model behaviour, validating output formats, filtering inputs and outputs, enforcing least privilege, requiring human approval for high-risk actions, segregating external content, and running adversarial testing. (Source: OWASP GenAI LLM01 page)

**[fact]** Microsoft distinguishes user-prompt attacks from document attacks, which implies that grounded or retrieval-heavy systems need separate controls for user-entered content and third-party content. (Source: Microsoft Prompt Shields documentation)

**[fact]** CaMeL's core design principle is that untrusted data must not control program flow and must not be able to exfiltrate protected data through tool calls. (Source: arXiv:2503.18813)
...
**Technical lens**
- **[fact]** The core technical failure is instruction/data ambiguity inside natural-language interfaces. That is why direct prompt injection, indirect prompt injection, and many-shot jailbreaking all rhyme: they are different ways of steering the same language-conditioned control loop. (Sources: Greshake et al.; Anthropic many-shot; CaMeL abstract)
- **[inference]** Architectural controls outperform prompt-only controls because they move enforcement into deterministic layers: capability checks, routing logic, tool permissions, and approval workflows.

**Economic lens**
- **[inference]** Stronger security usually costs capability, latency, or engineering complexity. Anthropic reports additional compute and some refusal trade-offs. CaMeL reports lower secure task completion than an undefended system. This means organisations must choose an explicit security-capability frontier rather than assume free safety.

**Regulatory and governance lens**
- **[fact]** OWASP and NIST now treat prompt injection as part of the formal security and risk-governance landscape rather than a niche red-team curiosity. (Sources: OWASP LLM01; NIST AI 100-2)
- **[inference]** That shift matters operationally: once the issue appears in standards and enterprise controls, boards and security teams can no longer dismiss it as an experimental model quirk.

**Historical lens**
- **[fact]** Simon Willison's writing shows the field's argument has been stable since 2022 on one central point: prompt injection is hard because the model cannot cleanly separate trusted instructions from untrusted text. What changed by 2025 is the magnitude of the consequences as agents gained tools and ambient context. (Source: Simon Willison prompt injection series)

**Behavioural lens**
- **[inference]** Human operators systematically over-trust coherent language output. This compounds prompt injection risk because a compromised agent may produce plausible justifications for malicious or incorrect actions, reducing the chance of timely detection unless approval and audit trails are designed in.

### §6 Synthesis

**Executive summary:**

**[fact]** Prompt injection is a structural security vulnerability in agentic AI systems, not a narrow content-safety bug, and the evidence from 2023-2025 shows it has already crossed from theory into deployed-system incidents and observed malicious use. (Sources: OWASP LLM01; Greshake et al. 2023; Unit 42 report)

**[fact]** The strongest public evidence supports active exploitation by researchers, bug hunters, and malicious content publishers using indirect prompt injection against browsing, retrieval, and tool-using systems; the public evidence for large-scale nation-state or organised-criminal prompt-injection campaigns remains comparatively thin. (Sources: Greshake et al. 2023; Unit 42 report; Simon Willison incident catalogue)

**[fact]** Defenders now include standards bodies, major model vendors, and security companies, but the best published results show partial risk reduction rather than elimination: strong layered defences can lower attack success sharply, yet adaptive attackers still bypass current systems. (Sources: Anthropic Constitutional Classifiers; Microsoft Prompt Shields; Lakera PINT benchmark; Zhan et al. 2025)

**[inference]** The research frontier therefore treats prompt injection as an architectural problem requiring blast-radius reduction, capability isolation, least privilege, and human approval for high-impact actions. (Sources: OWASP LLM01; arXiv:2503.18813; Simon Willison prompt injection series)

**Key findings:**

1. **[fact] High confidence:** Prompt injection is now best understood as an architectural vulnerability of language-driven control systems, because standards bodies, foundational papers, and vendor security research all show that agents still struggle to separate trusted instructions from untrusted data when they ingest external content or operate tools. (Sources: OWASP LLM01; Greshake et al. 2023; NIST AI 100-2)
2. **[fact] High confidence:** Indirect prompt injection is the most dangerous practical class for agentic systems because attackers can plant instructions in web pages, documents, source files, or tool outputs and influence an agent without ever gaining direct access to the main chat interface. (Sources: Greshake et al. 2023; OWASP LLM01; Unit 42 report)
3. **[fact] Medium confidence:** Public evidence shows real exploitation in deployed systems and web ecosystems, including disclosed Common Vulnerabilities and Exposures and Unit 42 telemetry, but the most strongly evidenced public attackers today are researchers, bug hunters, and malicious content publishers rather than clearly attributed nation-state prompt-injection campaigns. (Sources: Greshake et al. 2023; OWASP scenarios; Unit 42 report; Simon Willison incident catalogue)
4. **[fact] High confidence:** Defence work is active and increasingly mature across Anthropic, Microsoft, Google DeepMind, OWASP, NIST, and security vendors, but these efforts consistently frame prompt injection as a risk to be bounded with layers and architecture rather than a bug that can be fully patched with prompt engineering alone. (Sources: Anthropic many-shot jailbreaking; Anthropic Constitutional Classifiers; Microsoft Prompt Shields; OWASP LLM01)
5. **[fact] High confidence:** Published defences can materially reduce attack success in bounded settings - for example Anthropic's Constitutional Classifiers and Google DeepMind's capability-based dual-model defence architecture - yet those gains come with trade-offs in compute cost, refusal behaviour, latency, or task completion, which means security and capability remain coupled design choices. (Sources: Anthropic Constitutional Classifiers; arXiv:2503.18813; Lakera PINT benchmark)
6. **[fact] High confidence:** Adaptive evaluation is the critical research correction of 2025, because papers that let attackers optimise against the defence report bypasses of all tested indirect prompt injection defences and show that static benchmark wins are not strong evidence of real robustness. (Sources: Zhan et al. 2025; Google DeepMind Gemini security paper)
7. **[inference] Medium confidence:** The field's most credible near-term path is not universal prevention but constrained-agent design: least privilege, isolated tool execution, deterministic policy checks, segmentation of untrusted content, and human approval for irreversible or high-value actions. (Sources: OWASP LLM01; arXiv:2503.18813; Microsoft Prompt Shields; Simon Willison prompt injection series)
8. **[fact] High confidence:** The most important open problems are instruction/data separation, multimodal prompt injection, secure long-term memory, realistic adaptive benchmarks, and formally enforceable constraints on tool use, which means the research community still does not have a generally accepted definition of what a "solved" prompt injection defence would look like for general-purpose agents. (Sources: arXiv:2503.18813; Zhan et al. 2025; Anthropic many-shot jailbreaking; OWASP LLM01)

**Evidence map:**

| Claim | Source | Confidence | Notes |
|---|---|---|---|
| Prompt injection is an architectural vulnerability, not just a content-safety issue. | OWASP LLM01; Greshake et al. 2023; NIST AI 100-2 | high | All three frame the issue as a system-level security problem tied to how language interfaces are used. |
| Indirect prompt injection is the most dangerous class for agents. | Greshake et al. 2023; OWASP LLM01; Unit 42 report | high | All three describe attacker-controlled external content steering tool-using systems. |
| Public evidence of real exploitation is strongest for researchers, bug hunters, and malicious content publishers. | Greshake et al. 2023; OWASP scenarios; Unit 42 telemetry; Simon Willison incident catalogue | medium | Evidence of malicious deployment is strong, but attribution to specific nation-state or organised-criminal campaigns is still sparse. |
| Defenders have shifted to layered and architectural controls rather than prompt-only fixes. | Anthropic many-shot; Anthropic Constitutional Classifiers; Microsoft Prompt Shields; CaMeL paper; OWASP LLM01 | high | Diverse defenders converge on filters, least privilege, capability controls, and approval gates. |
| Defences reduce risk but impose trade-offs. | Anthropic Constitutional Classifiers; CaMeL paper; Lakera PINT benchmark | high | Metrics show real gains alongside compute, refusal, or capability costs. |
| Adaptive evaluation overturns optimistic defence claims. | Zhan et al. 2025; Google DeepMind Gemini security paper | high | Adaptive attack testing is now central to credible evaluation. |
| Constrained-agent design is the most credible near-term mitigation path. | OWASP LLM01; CaMeL paper; Microsoft Prompt Shields; Simon Willison series | medium | Strongly supported as a design recommendation, though not proven as a universal solution. |
| Key open problems remain unresolved across core architecture and evaluation. | CaMeL paper; Zhan et al. 2025; Anthropic many-shot; OWASP LLM01; Simon Willison series | high | Multiple independent sources agree that current approaches remain incomplete. |
...
The evidence was weighted toward documents that either define the threat model or report measured outcomes. **[fact]** OWASP, NIST, and Greshake et al. establish the canonical threat definition; Anthropic, Google DeepMind, Microsoft, and Lakera provide measured defence evidence; and Unit 42 provides important evidence that indirect prompt injection has moved into observed malicious deployment. (Sources: OWASP LLM01; NIST AI 100-2; Greshake et al. 2023; Anthropic Constitutional Classifiers; Microsoft Prompt Shields; arXiv:2503.18813; Lakera PINT benchmark; Unit 42 report) These sources agree on the central direction of travel: prompt injection has become more serious as agents gained tools and ambient context. Where the evidence is thinner is not on the existence of the vulnerability, but on attacker attribution and prevalence outside disclosed incidents and red-team settings. That is why the synthesis distinguishes clearly between "the weakness is real and exploited" and "we can quantify which threat-actor class uses it most."

The major trade-off is between utility and controllability. Stronger system prompts and detectors preserve capability but remain bypassable. Architectural separation, deterministic policy checks, and human approval reduce risk more credibly but cost latency, engineering effort, or autonomy. **[inference]** The literature increasingly favours explicit risk budgeting over promises of perfect prevention. (Sources: Anthropic Constitutional Classifiers; arXiv:2503.18813; Zhan et al. 2025)

**Risks, gaps, uncertainties:**

- Public attribution data is weak for nation-state and organised-criminal prompt-injection campaigns compared with researcher disclosures and vendor-reported incidents.
- Multimodal prompt injection has credible demonstrations, but far less production evidence than text and document-based indirect prompt injection.
- Benchmark scores for detectors can overstate robustness if attackers are not adapting to the defence or if the benchmark distribution differs from production workloads.
- Vendor write-ups can mix product positioning with research results, so they were weighted most heavily when they included concrete metrics or limitations.
- MITRE ATLAS is clearly relevant but difficult to quote directly from the current site; accessible primary detail came through linked framework references rather than richly extractable ATLAS pages.
...
**[fact]** Prompt injection is a structural security vulnerability in agentic AI systems rather than a narrow content-safety bug, and the published evidence now spans both foundational papers and deployed-system incident reporting. (Sources: https://genai.owasp.org/llmrisk/llm01-prompt-injection/ ; https://arxiv.org/abs/2302.12173 ; https://unit42.paloaltonetworks.com/ai-agent-prompt-injection/)

**[fact]** The strongest public evidence today points to active exploitation by researchers, bug hunters, and malicious content publishers using indirect prompt injection against browsing, retrieval, and tool-using systems, while public attribution to large-scale nation-state or organised-criminal prompt-injection campaigns remains relatively sparse. (Sources: https://arxiv.org/abs/2302.12173 ; https://unit42.paloaltonetworks.com/ai-agent-prompt-injection/ ; https://simonwillison.net/series/prompt-injection/)

**[fact]** Defenders now include standards bodies, model vendors, and specialist security companies, but the best published measurements show partial risk reduction rather than elimination because adaptive attackers still bypass current defences. (Sources: https://www.anthropic.com/news/constitutional-classifiers ; https://learn.microsoft.com/en-us/azure/ai-services/content-safety/concepts/jailbreak-detection ; https://github.com/lakeraai/pint-benchmark ; https://aclanthology.org/2025.findings-naacl.395/)

**[inference]** The most defensible near-term response is therefore to design agents around blast-radius reduction, capability isolation, least privilege, and explicit approval gates for high-impact actions. (Sources: https://genai.owasp.org/llmrisk/llm01-prompt-injection/ ; https://arxiv.org/abs/2503.18813 ; https://simonwillison.net/series/prompt-injection/)

### Key Findings

1. **[inference] Confidence: high.** Prompt injection should be treated as an architectural vulnerability of language-driven control systems because the core failure mode is the system's inability to reliably keep trusted instructions separate from untrusted content once external data and tools enter the loop. (Sources: https://genai.owasp.org/llmrisk/llm01-prompt-injection/ ; https://arxiv.org/abs/2302.12173 ; https://nvlpubs.nist.gov/nistpubs/ai/NIST.AI.100-2e2023.pdf)
2. **[inference] Confidence: high.** Indirect prompt injection is the most dangerous practical class for agentic systems because an attacker can steer the agent through web pages, documents, source files, or tool outputs without first winning access to the main chat interface. (Sources: https://arxiv.org/abs/2302.12173 ; https://genai.owasp.org/llmrisk/llm01-prompt-injection/ ; https://unit42.paloaltonetworks.com/ai-agent-prompt-injection/)
3. **[fact] Confidence: medium.** Public evidence already shows exploitation in deployed systems and web ecosystems, including disclosed vulnerabilities and observed malicious web payloads, but the best-evidenced public attackers are still researchers, bug hunters, and malicious content publishers rather than clearly attributed nation-state prompt-injection campaigns. (Sources: https://arxiv.org/abs/2302.12173 ; https://genai.owasp.org/llmrisk/llm01-prompt-injection/ ; https://unit42.paloaltonetworks.com/ai-agent-prompt-injection/ ; https://simonwillison.net/series/prompt-injection/)
4. **[fact] Confidence: high.** Defence work is active across Anthropic, Microsoft, Google DeepMind, Open Worldwide Application Security Project, National Institute of Standards and Technology, and specialist security vendors, and these sources consistently frame prompt injection as a risk to be bounded with layers and architecture rather than a bug that prompt wording alone can patch away. (Sources: https://www.anthropic.com/research/many-shot-jailbreaking ; https://www.anthropic.com/news/constitutional-classifiers ; https://learn.microsoft.com/en-us/azure/ai-services/content-safety/concepts/jailbreak-detection ; https://genai.owasp.org/llmrisk/llm01-prompt-injection/)
5. **[fact] Confidence: high.** Published defences can materially reduce attack success in bounded settings, but those gains come with trade-offs in compute cost, refusal behaviour, latency, or task completion, which means security and capability remain coupled design choices rather than independent objectives. (Sources: https://www.anthropic.com/news/constitutional-classifiers ; https://arxiv.org/abs/2503.18813 ; https://github.com/lakeraai/pint-benchmark)
6. **[inference] Confidence: high.** Adaptive evaluation is the most important research correction of 2025 because papers that let attackers optimise against the defence show that benchmark wins alone are weak evidence of real robustness under adversarial pressure. (Sources: https://aclanthology.org/2025.findings-naacl.395/ ; https://storage.googleapis.com/deepmind-media/Security%20and%20Privacy/Gemini_Security_Paper.pdf)
7. **[inference] Confidence: medium.** Constrained-agent design is the most credible near-term path because least privilege, isolated tool execution, deterministic policy checks, segmentation of untrusted content, and human approval for irreversible actions all reduce blast radius even when the model itself remains imperfectly robust. (Sources: https://genai.owasp.org/llmrisk/llm01-prompt-injection/ ; https://arxiv.org/abs/2503.18813 ; https://learn.microsoft.com/en-us/azure/ai-services/content-safety/concepts/jailbreak-detection ; https://simonwillison.net/series/prompt-injection/)
8. **[fact] Confidence: high.** The most important open problems remain instruction/data separation, multimodal prompt injection, secure long-term memory, realistic adaptive benchmarks, and formally enforceable tool constraints, so the field still lacks a generally accepted definition of what a solved prompt-injection defence would look like for general-purpose agents. (Sources: https://arxiv.org/abs/2503.18813 ; https://aclanthology.org/2025.findings-naacl.395/ ; https://www.anthropic.com/research/many-shot-jailbreaking ; https://genai.owasp.org/llmrisk/llm01-prompt-injection/)
...
The evidence was weighted toward documents that either define the threat model or report measured outcomes. OWASP, NIST, and Greshake et al. establish the canonical threat definition; Anthropic, Google DeepMind, Microsoft, and Lakera provide measured defence evidence; Unit 42 provides important evidence that indirect prompt injection has moved into observed malicious deployment. These sources agree on the central direction of travel: prompt injection has become more serious as agents gained tools and ambient context. Where the evidence is thinner is not on the existence of the vulnerability, but on attacker attribution and prevalence outside disclosed incidents and red-team settings. That is why the synthesis distinguishes clearly between "the weakness is real and exploited" and "we can quantify which threat-actor class uses it most."

The major trade-off is between utility and controllability. Stronger system prompts and detectors preserve capability but remain bypassable. Architectural separation, deterministic policy checks, and human approval reduce risk more credibly but cost latency, engineering effort, or autonomy. The literature increasingly favours explicit risk budgeting over promises of perfect prevention.
