...
- Access note: the seeded MIT Press locators for Clarke et al. and Baier and Katoen were checked, but accessible extracted claims came from PRISM, Storm, and SPIN documentation plus accessible papers.
- Access note: the seeded EUR-Lex page was checked, but extractable Article 12 wording was taken from the European Commission service desk mirror of the same provision.
...
- [fact] The available tool sources do not support the strong claim that stochastic or agentic systems are beyond all formal analysis, because PRISM and Storm explicitly support probabilistic and nondeterministic models. [source: https://www.prismmodelchecker.org/; https://www.stormchecker.org/]
...
Probabilistic model checking shows that stochastic systems are not outside formal methods altogether, yet the tractable object is a finitised Markov-style abstraction such as an MDP rather than the full semantic content of an open-ended agent run. [fact; source: https://www.prismmodelchecker.org/; https://www.prismmodelchecker.org/manual/ThePRISMLanguage/LocalNondeterminism; https://www.stormchecker.org/]
...
1. **A production-grade audit trail for an agentic pipeline must capture conversation identity, model identity, prompt and instruction state, retrieval lineage, tool-call lineage, outputs, and errors, because current observability standards split those facts across traces, events, and attributes rather than one canonical log record.** ([inference]; high confidence; source: https://opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-spans/; https://opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-events/; https://opentelemetry.io/docs/specs/semconv/registry/attributes/gen-ai/)
2. **The European Union Artificial Intelligence Act treats traceability as an automatic lifetime logging requirement for high-risk systems, which means post hoc textual explanations alone are insufficient where the system's functioning must later be reconstructed for monitoring or investigation.** ([fact]; high confidence; source: https://ai-act-service-desk.ec.europa.eu/en/ai-act/article-12)
3. **Current mainstream formal-verification tooling already supports probabilistic and nondeterministic state-transition models such as Markov Decision Processes, so the hard boundary is not the presence of randomness itself but the need to first compress an open-ended workflow into a finite model.** ([fact]; high confidence; source: https://www.prismmodelchecker.org/; https://www.prismmodelchecker.org/manual/Main/AllOnOnePage; https://www.prismmodelchecker.org/manual/ThePRISMLanguage/LocalNondeterminism; https://www.stormchecker.org/)
...
6. **Approximate techniques such as statistical model checking help when explicit probabilistic verification becomes infeasible, but they weaken assurance from exhaustive proof to sampled approximation and remain an awkward fit for models with unresolved nondeterministic choices.** ([fact]; high confidence; source: https://www.prismmodelchecker.org/manual/RunningPRISM/StatisticalModelChecking)
...
|---|---|---|---|
| [fact] Audit reconstruction for agentic pipelines requires prompt, output, retrieval, tool, and configuration lineage. | https://opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-spans/ ; https://opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-events/ ; https://opentelemetry.io/docs/specs/semconv/registry/attributes/gen-ai/ | high | direct specification fields |
| [fact] High-risk AI systems must automatically record events over their lifetime and support traceability appropriate to intended purpose. | https://ai-act-service-desk.ec.europa.eu/en/ai-act/article-12 | high | direct statutory wording |
| [fact] Probabilistic model checkers directly support Markov chains, MDPs, and related finite-state models. | https://www.prismmodelchecker.org/ ; https://www.prismmodelchecker.org/manual/Main/AllOnOnePage ; https://www.stormchecker.org/ | high | direct tool scope |
...
| [fact] Statistical model checking is approximate and is not well suited to unresolved nondeterminism in MDPs. | https://www.prismmodelchecker.org/manual/RunningPRISM/StatisticalModelChecking | high | direct limitation |
...
The evidence does not support a simplistic claim that deterministic systems are verifiable and agentic systems are not. PRISM and Storm show that probabilistic and nondeterministic models are legitimate formal-verification targets. [fact; source: https://www.prismmodelchecker.org/; https://www.stormchecker.org/]
...
- The seeded textbook pages from MIT Press were not extractable in this environment, so the formal-method evidence here comes from tool documentation and accessible papers rather than from the textbooks themselves. [fact; source: https://mitpress.mit.edu/9780262038553/model-checking/; https://mitpress.mit.edu/9780262026499/principles-of-model-checking/]
