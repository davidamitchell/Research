...
- [inference] A concrete ERM counterexample can be built with binary features `(C,S)` and label `Y`, where the invariant feature `C` predicts `Y` with error `0.20` in both training environments, the spurious feature `S` predicts `Y` with errors `0.05` and `0.10` in those same environments, and test-time environment change flips the spurious relation so that `S` now predicts `Y` with error `0.90`. [source: https://arxiv.org/abs/1907.02893; https://arxiv.org/abs/2102.11107]
- [inference] Over the pooled training distribution, ERM rationally selects the `S`-based rule because its average empirical risk, `0.075`, is lower than the invariant rule's `0.20`, yet the selected rule is causally wrong because its success depends on a non-stable correlation rather than on the mechanism generating `Y`. [source: https://arxiv.org/abs/1907.02893; https://arxiv.org/abs/1501.01332]
...
Invariant Risk Minimisation (IRM) is the formal correction because it seeks representations whose optimal classifier is stable across environments, and gradient-descent simplicity bias helps explain why shortcut ERM solutions are often found first in practice. [fact; source: https://arxiv.org/abs/1907.02893; https://openreview.net/forum?id=VCnuSuDSHv]
...
5. **IRM is a formal correction to ERM's blind spot because it searches for representations whose optimal classifier is invariant across training environments, which ties the learning objective more closely to stable causal structure.** ([fact]; high confidence; source: https://arxiv.org/abs/1907.02893; https://arxiv.org/abs/2102.11107)
...
| [fact] Low observational risk does not answer intervention or counterfactual questions. | https://arxiv.org/abs/1801.04016; https://arxiv.org/abs/1501.01332 | high | multiple causal sources agree |
...
| [fact] IRM adds an invariance criterion across environments. | https://arxiv.org/abs/1907.02893; https://arxiv.org/abs/2102.11107 | high | direct objective statement plus causal framing |
