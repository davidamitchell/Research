...
[inference] TOC practitioners note that in manufacturing, the constraint is often physical (a machine or process step). In knowledge work, including software delivery, the constraint is more often a policy: a review cadence, an approval chain, or a governance requirement. Goldratt's Five Focusing Steps warn explicitly against elevating capacity (adding resources) before exploiting and subordinating non-constraint activity to the actual constraint, noting that premature capacity investment typically reveals 30% or more hidden capacity already exists but is blocked by policy. Source: [TOC Institute, Five Focusing Steps](https://www.tocinstitute.org/five-focusing-steps.html); [Mitchell (2026) Backpressure and TOC](https://davidamitchell.github.io/Research/research/2026-04-01-backpressure-theory-of-constraints.html).
...
[fact] Value stream mapping (VSM) applied to knowledge-work approval processes consistently shows that active processing time (value-adding time) accounts for 1% to 10% of total elapsed time, with the remaining 90% to 99% attributable to wait time (queueing for approval, review, or decision). Source: [Org Topologies case study](https://www.orgtopologies.com/post/case-study-from-component-teams-to-team-topologies-to-fast-agile).

[fact] The Org Topologies case study based on James Shore's analysis of a 200-engineer, 300-person research and development (R&D) organisation with tightly coupled component teams found 97% waste or wait time from VSM analysis, meaning work that required 2 to 3 days of actual effort consumed months of calendar time. This organisation was "getting killed by cross-team dependencies," each of which required inter-team handoffs and approval sequences. Source: [Org Topologies case study](https://www.orgtopologies.com/post/case-study-from-component-teams-to-team-topologies-to-fast-agile).

[inference] Queueing theory as applied to product development by Donald Reinertsen predicts that queue length and therefore lead time grows non-linearly as the approval node approaches capacity saturation. At 80% utilisation, queue time roughly doubles for a single-server queue; at 90% utilisation it roughly quadruples. In governance-heavy organisations, approval nodes (committees, boards, sign-off chains) frequently operate near or above their review-bandwidth capacity. The non-linear effect means that small reductions in approval frequency or complexity produce disproportionately large improvements in lead time. Source: [DORA, Streamlining Change Approval](https://dora.dev/capabilities/streamlining-change-approval/); [DORA Metrics Four Keys](https://dora.dev/guides/dora-metrics-four-keys/).
...
[inference] Funding gates create throughput instability by periodically stopping and starting work streams. During gate closure, demand accumulates as a batch; when the gate opens, a large volume of work enters the delivery system simultaneously, creating a demand spike that overwhelms execution capacity even when governance approval is not the steady-state bottleneck. This batch arrival pattern is structurally similar to the large-batch problem that Reinertsen identifies as a primary cause of queueing blowout. Source: [Mitchell (2026) Funding authority and delivery-risk accountability split](https://davidamitchell.github.io/Research/research/2026-05-23-funding-authority-delivery-capability-risk-accountability-split.html); [Mitchell (2026) Operating model synthesis P1](https://davidamitchell.github.io/Research/research/2026-05-29-split-authority-p1-operating-model-synthesis.html).
...
3. VSM studies consistently show 90% to 99% of cycle time is wait time in approval-dependent processes. [fact; source: https://www.orgtopologies.com/post/case-study-from-component-teams-to-team-topologies-to-fast-agile]
...
acronym_audit: passed
  -- TOC: expanded as Theory of Constraints on first use
  -- DORA: expanded as DevOps Research and Assessment on first use
  -- VSM: expanded as value stream mapping on first use
  -- WIP: expanded as work-in-progress on first use
  -- CAB: expanded as Change Advisory Board on first use
  -- R&D: expanded as research and development on first use
  -- POC: not used
...
The five candidate constraints form a hierarchy rather than a set of independent alternatives. Fragmented decision rights are the structural condition; approval latency is the direct mechanism; dependency coupling is the amplifier; and funding gates are the periodic, coarser-cycle variant. Capacity shortage is the only independent candidate, but evidence does not support it as the primary system-level constraint in split-authority organisations that have not yet reduced governance friction. [inference; source: https://www.tocinstitute.org/five-focusing-steps.html; https://dora.dev/capabilities/streamlining-change-approval/]

The most compelling evidence is DORA's negative correlation finding, because it is multi-firm, multi-industry, and longitudinal, and because it directly tests the governance-approval variable rather than inferring it. The mechanistic support from TOC policy-constraint theory and the VSM case study evidence are consistent with the DORA finding. Prior completed repository items on governance controls effectiveness and governance failure mechanisms provide additional causal texture: controls become overhead when applied uniformly at high volume, precisely the condition in split-authority systems where every work item must pass through a multi-stakeholder approval chain. [inference; source: https://dora.dev/capabilities/streamlining-change-approval/; https://davidamitchell.github.io/Research/research/2026-05-23-governance-controls-effectiveness-conditions.html]

No strong rival explanation supports capacity shortage as primary. The TOC framework explicitly warns against this misdiagnosis. The IT throughput constraint completed item notes that governance overhead consumes capacity and that the approval queue hides latent capacity, which is the mechanistic pathway by which a capacity-shortage appearance is actually a governance-constraint reality. [inference; source: https://www.tocinstitute.org/five-focusing-steps.html; https://davidamitchell.github.io/Research/research/2026-05-16-it-throughput-constraint-magnitude-and-debt-accumulation-rate.html]

The self-reinforcing feedback loop between approval latency and batch-size growth is the instability mechanism. Systems in this loop are not merely slow; they are dynamically unstable: any increase in governance overhead accelerates the feedback, driving throughput further down and batch size further up, until an external intervention (governance redesign, senior escalation, or accumulated delivery failure) breaks the cycle. [inference; source: https://dora.dev/capabilities/streamlining-change-approval/; https://davidamitchell.github.io/Research/research/2026-05-23-governance-failure-mechanisms-bureaucracy-circumvention.html]
