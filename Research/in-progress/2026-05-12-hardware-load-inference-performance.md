...
- [fact; source: https://arxiv.org/abs/2403.02310] Agrawal et al. say each LLM serving request has a prompt-processing prefill phase that has high latency but high GPU-compute utilization, and a token-by-token decode phase that has lower latency but lower compute utilization, which makes batching valuable for throughput but risky for latency if the phases are mixed poorly.
...
1. **Large Language Model serving slowdowns under load follow threshold-based shifts in scheduler behavior, because prefill and decode phases stress different resources and operators explicitly trade inter-token latency, time to first token, and total throughput through batching policy rather than accepting a simple linear slowdown with higher Graphics Processing Unit utilization.** ([inference]; high confidence; source: https://arxiv.org/abs/2403.02310; https://docs.vllm.ai/en/latest/configuration/optimization/; https://huggingface.co/docs/text-generation-inference/main/en/architecture)
2. **Key-value cache pressure is a major bottleneck under concurrent inference, because better memory management alone delivered 2 to 4 times higher throughput at similar latency in vLLM, while insufficient cache space forces request preemption and recomputation that directly worsens end-to-end latency.** ([fact]; high confidence; source: https://arxiv.org/abs/2309.06180; https://docs.vllm.ai/en/latest/configuration/optimization/)
3. **Central Processing Unit overhead still materially affects Graphics Processing Unit inference performance, especially at low batch sizes and on multi-socket or tightly coupled platforms, because kernel-launch and queuing overhead can keep systems CPU-bound even when substantial accelerator capacity remains available.** ([fact]; medium confidence; source: https://arxiv.org/abs/2504.11750; https://docs.vllm.ai/en/latest/configuration/optimization/; https://github.com/ggml-org/llama.cpp/tree/master/tools/llama-bench)
4. **The studied inference systems handle contention through explicit queueing, batching, and concurrency controls, because TGI, Ollama, vLLM, and llama.cpp all expose knobs for request buffering, token budgets, memory residency, or parallel slots that operators must tune to keep latency and throughput inside target envelopes.** ([fact]; high confidence; source: https://huggingface.co/docs/text-generation-inference/main/en/architecture; https://docs.ollama.com/faq; https://github.com/ollama/ollama/blob/main/envconfig/config.go; https://docs.vllm.ai/en/latest/configuration/optimization/; https://github.com/ggml-org/llama.cpp/blob/master/tools/server/README.md)
5. **The consulted evidence for load-sensitive output consistency comes from batch-sensitive numerical divergence, because vLLM says changed batching can alter sampled tokens and Yuan et al. show that changing batch size, GPU count, or GPU type can shift task accuracy by up to 9 percent even under greedy decoding.** ([fact]; high confidence; source: https://docs.vllm.ai/en/v0.7.0/getting_started/faq.html; https://arxiv.org/abs/2506.09501)
6. **The consulted evidence does not strongly support a claim that high utilization alone degrades semantic quality when the numerical path is fixed, because the documented quality risks instead arise when operators change precision, quantize the key-value cache, or let live batching alter the execution path seen by the request.** ([inference]; medium confidence; source: https://docs.ollama.com/faq; https://docs.vllm.ai/en/v0.7.0/getting_started/faq.html; https://thinkingmachines.ai/blog/defeating-nondeterminism-in-llm-inference/)
7. **Production agent reliability therefore depends on workload-class isolation, bounded concurrency, warm model residency, and deterministic external checks for high-consequence steps, because latency spikes and occasional token drift can propagate across multi-step workflows faster than human operators can intervene.** ([inference]; medium confidence; source: https://docs.vllm.ai/en/latest/configuration/optimization/; https://docs.ollama.com/faq; https://huggingface.co/docs/text-generation-inference/main/en/architecture; https://davidamitchell.github.io/Research/research/2026-04-28-uelgf-agentic-ai-specific-risks-runtime-monitoring.html; https://davidamitchell.github.io/Research/research/2026-05-08-integrated-cascading-failure-agentic-vs-generative-ai-risk.html; https://davidamitchell.github.io/Research/research/2026-05-09-llm-determinism-limits-temperature-zero.html)
