...
- [fact] In mid-to-large software-dependent organisations, the coordination and governance layer created around software delivery is best estimated at at least the high teens of headcount and often around one quarter to one third, and falling coding costs will compress that layer but not eliminate it.
...
- [inference] The most likely organisational outcome is not a flat company with no management. It is a smaller execution layer around stronger platforms, self-service automation, and a more concentrated human governance core.
...
In mid-to-large software-dependent organisations, the coordination and governance layer created around software delivery is best estimated at at least the high teens of headcount and often around one quarter to one third, and falling coding costs will compress that layer but not eliminate it. The roles most exposed are translation and queue-management roles whose main output is turning intent into backlogs, plans, status artifacts, and handoffs. The roles least exposed are the ones that encode authority, standards, risk appetite, and external accountability. The most likely future shape is therefore not a flat firm with no management, but smaller AI-augmented execution cells operating inside a more concentrated human governance core.

### Key Findings

1. **High confidence:** In mid-to-large software-dependent organisations, explicit coordination and governance roles are best estimated at at least the high teens of headcount and often roughly one quarter to one third once team-level, train-level, architecture, release, and management layers are counted together.
2. **High confidence:** Product Owner, Scrum Master, Business Analyst, project or programme manager, delivery manager, release manager, change manager, and intermediary solution-architecture roles exist largely because business intent and software execution are separated by translation, validation, batching, and queueing costs.
3. **High confidence:** Transaction-cost logic predicts that materially cheaper coding should reduce the hierarchy devoted to allocating scarce engineering labor, while preserving and often concentrating the hierarchy that owns standards, compliance, risk acceptance, and residual decision rights.
4. **High confidence:** ERP, cloud, and low-code and no-code adoption all reduced some local execution work but replaced it with new central structures for standards, enablement, platform governance, shared services, or Centers of Excellence rather than eliminating coordination altogether.
5. **Medium to high confidence:** Current AI evidence shows large gains on routine software tasks and measurable reductions in project-management friction, but it does not justify the stronger claim that complex enterprise software delivery is already near-zero cost from idea to production.
6. **High confidence:** The first coordination work to compress is artifact transformation, including backlog drafting, routine status reporting, repetitive release administration, and simple compliance evidence gathering, while judgment-intensive work remains human-heavy for longer.
7. **Medium confidence:** Agile rituals, estimation cycles, and roadmap ceremonies will thin sharply wherever they exist mainly to ration scarce developers, but they will persist wherever they encode commitments, dependency management, risk review, or regulated change control.
8. **Medium to high confidence:** The most plausible future organisational equilibrium is smaller software-execution cells around stronger platforms and guardrails, with regulated firms converging on a dual-core model of AI-heavy delivery plus concentrated governance rather than managerial collapse.
...
| Falling coding cost should shrink throughput-allocation hierarchy but not governance hierarchy | Seeded Coase and Williamson source pointers checked directly, plus synthesis from this item's external analogues | high | Theory claim is inferential because seeded primary texts were not directly quotable in this environment |
...
The evidence supports a bottleneck-migration story rather than a disappearance story. Current software organisations name coordination roles explicitly because business intent, technical implementation, and organisational accountability are split across different people and time horizons. AI already compresses routine coding, documentation, and some project-management-adjacent work, so the economic rationale for large translation layers weakens first. Historical analogues show that this does not produce a coordination-free organisation; it produces a different coordination layer centered on standards, platforms, guardrails, and risk ownership. The practical implication is that firms should redesign around thinner translation layers and clearer decision rights, not assume that a cheaper production technology makes governance optional.

### Risks, Gaps, and Uncertainties

- No clean public dataset decomposes software-dependent firms into builder versus coordinator headcount, so the percentage answer is a bounded estimate rather than a census fact.
- Several seeded sources were inaccessible or unusable in this environment: the Coase Wiley page returned 403, the Gartner low-code page returned 403, and the seeded Standish and Wired links returned 404.
- Some AI-productivity evidence comes from vendor-published sources such as Cognition and Cursor, which are useful directional signals but weaker than controlled experiments or independent working papers.
- Public evidence remains much stronger for routine coding and documentation tasks than for complex cross-system, regulated, or politically contested enterprise change.

### Open Questions

- What is the best direct empirical method for measuring builder versus coordinator headcount in large software-dependent firms?
- At what point do AI agents reduce not just coding effort but also the demand for project-management and architecture-intermediary roles in regulated enterprises?
- Which governance model replaces the investment board or project front door when software throughput is no longer the binding constraint?
