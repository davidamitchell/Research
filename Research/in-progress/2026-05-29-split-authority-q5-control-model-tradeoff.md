...
The IT Service Management (ITSM) framework ITIL 4 Change Enablement reaches the same assignment from a different starting point: standard changes (low-risk, documented, repeatable) are pre-authorised; normal changes require assessed per-instance approval; emergency changes require the Emergency Change Advisory Board (ECAB). [inference; source: https://www.axelos.com/certifications/itil-service-management/what-is-itil] This maps standard = Class 1/post-hoc, normal = Class 2/bounded delegation, emergency = Class 3/pre-approval.

DORA (DevOps Research and Assessment) research confirms that elite delivery organisations achieve high deployment frequency for standard work by pre-authorising low-risk changes, while retaining review for high-risk changes. [inference; source: https://dora.dev/guides/dora-metrics-four-keys/]
...
The Human-in-the-Loop (HITL) capacity thresholds item in this corpus found that when review volume exceeds human capacity, nominal pre-approval collapses into rubber-stamping, losing meaningful challenge while retaining all the throughput cost of pre-approval. [inference; source: https://davidamitchell.github.io/Research/research/2026-05-20-hitl-capacity-thresholds-in-banking-compliance.html] This is an additional argument for restricting pre-approval to Class 3 work: pre-approval applied to Class 1 and 2 volume quickly exceeds meaningful review capacity, producing the worst of both patterns (high latency, low risk reduction).
...
acronym_audit: passed
  ...
  - ITIL: expanded on first use as IT Service Management (ITSM) framework ITIL
  ...
  - DORA: expanded on first use as DevOps Research and Assessment (DORA)
