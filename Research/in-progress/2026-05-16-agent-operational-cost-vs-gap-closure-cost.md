...
What is the fully loaded operational cost of a production do-mode Artificial Intelligence (AI) agent, meaning an agent that works around a missing system capability instead of eliminating it, relative to the cost of closing the underlying systems capability gap via software delivery (build-mode) across three-year and five-year horizons, and under what conditions does do-mode investment produce positive return relative to build-mode investment?

## Scope

**In scope:**
- Cost model components for production do-mode agents: build, inference, harness operations, governance, and failure recovery.
- Cost model components for build-mode closure of integration, functionality, and data access debt classes.
...
Recurring do-mode work only outperforms system change when the underlying capability gap is too expensive, too uncertain, or too unstable to close within the planning horizon. [inference; source: https://cloud.google.com/resources/content/dora-roi-of-ai-assisted-software-development; https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-05-13-agent-process-reliability-architecture.md]
...
1. Compile production cost evidence for do-mode agents by complexity and task type, including inference, governance, and recovery overhead.
2. Compile delivery-cost evidence for closing underlying systems capability gaps with AI-assisted software engineering.
3. Model breakeven points and sensitivity to productivity multipliers for integration debt, functionality debt, and data access debt.
...
- Question: compare the fully loaded cost of operating a production do-mode Artificial Intelligence (AI) agent against the cost of removing the same capability gap through software delivery over three-year and five-year horizons.
- Scope: do-mode means the agent keeps doing the missing work operationally; build-mode means software delivery closes the missing integration, functionality, or data-access capability once in the underlying stack.
...
```text
What is the cost of operating a do-mode AI agent versus closing the gap in software?
|
|-- Q1. What costs are unavoidable for a production do-mode agent?
...
|
|-- Q2. What delivery-cost assumptions are defensible for build-mode closure?
...
|
|-- Q3. How do common debt classes differ?
|   |-- Q3a. What is a representative integration-debt closure effort?
|   |-- Q3b. What is a representative functionality-debt closure effort?
|   `-- Q3c. What is a representative data-access-debt closure effort?
|
`-- Q4. Under what conditions does do-mode produce positive return?
    |-- Q4a. What are the three-year breakeven thresholds?
    |-- Q4b. What are the five-year breakeven thresholds?
    `-- Q4c. Which contextual factors favor do-mode as a bridge rather than an end state?
```
...
- [inference] A reasonable multiplier band for planning is therefore 1.10x as a conservative organizational case, 1.26x as a central field-evidence case, and 1.56x as an optimistic bounded-task case. Source: https://cloud.google.com/resources/content/dora-impact-of-gen-ai-software-development; https://www.microsoft.com/en-us/research/publication/the-effects-of-generative-ai-on-high-skilled-work-evidence-from-three-field-experiments-with-software-developers/; https://arxiv.org/abs/2302.06590. Source class: inference from primary productivity studies.

#### Q2c - Maintenance after build-mode closure
...
- [inference] The decisive comparison is therefore not "agent tokens versus developer salary" but "recurring governed workaround cost versus one-time closure cost plus maintenance." Source: https://cloud.google.com/resources/content/dora-roi-of-ai-assisted-software-development; https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-05-13-agent-process-reliability-architecture.md.
- [inference] Because the operational workaround continues to carry review, governance, and recovery labor every year, do-mode only wins when closure effort is unusually large or the task is not yet stable enough to encode safely. Source: https://doi.org/10.6028/NIST.AI.100-1; https://dora.dev/research/2024/dora-report/; https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-05-09-hybrid-architecture-probabilistic-llm-deterministic-governance.md.

### §4 Consistency Check

- [fact] The GitHub Copilot experiment reports 55.8% faster completion on one bounded task, while Microsoft's field experiments report a 26.08% increase across real organizations; these figures are directionally consistent because one is task-bounded and one is field-averaged. Source: https://arxiv.org/abs/2302.06590; https://www.microsoft.com/en-us/research/publication/the-effects-of-generative-ai-on-high-skilled-work-evidence-from-three-field-experiments-with-software-developers/.
- [fact] DORA's productivity and stability findings are not contradictory to the build-mode thesis, because the report explicitly shows that local productivity gains do not remove the need for process redesign and control fundamentals. Source: https://dora.dev/research/2024/dora-report/; https://cloud.google.com/resources/content/dora-roi-of-ai-assisted-software-development.
...
- [inference] Technical lens: the more a workflow can be converted from probabilistic interpretation into deterministic execution, the more it behaves like build-mode capital formation rather than recurring operational expense. Source: https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-05-13-agent-process-reliability-architecture.md; https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-05-09-hybrid-architecture-probabilistic-llm-deterministic-governance.md.
...
- [inference] Regulatory lens: the stricter the control environment, the more the do-mode breakeven threshold moves rightward because required human review and evidence capture increase recurring operating cost. Source: https://doi.org/10.6028/NIST.AI.100-1; https://nvlpubs.nist.gov/nistpubs/ai/NIST.AI.600-1.pdf.
- [inference] Behavioural lens: low trust in generated code and explicit pull-request review patterns mean most organizations will not accept an unattended do-mode agent for consequential change, which limits the practical ceiling on labor savings. Source: https://dora.dev/research/2024/ai-preview/; https://docs.github.com/en/copilot/how-tos/copilot-on-github/use-copilot-agents/kick-off-a-task; https://docs.github.com/en/copilot/how-tos/copilot-on-github/use-copilot-agents/copilot-code-review.
...
Build-mode usually beats do-mode over three-year and five-year horizons when the underlying capability gap can be closed in roughly 14 to 21 delivery-months in a lean-to-standard operating model, and in roughly 20 to 30 delivery-months over five years. [inference; source: https://www.microsoft.com/en-us/research/publication/the-effects-of-generative-ai-on-high-skilled-work-evidence-from-three-field-experiments-with-software-developers/; https://www.bls.gov/oes/2023/may/oes151252.htm; https://www.bls.gov/news.release/ecec.t01.htm; https://cloud.google.com/resources/content/dora-roi-of-ai-assisted-software-development]

The dominant cost in a production do-mode agent is not inference but recurring human review, governance, and recovery labor, because published model prices remain low while trust, approval, and risk-management duties remain persistent. [inference; source: https://www.anthropic.com/pricing#api; https://cloud.google.com/gemini-enterprise-agent-platform/generative-ai/pricing; https://dora.dev/research/2024/ai-preview/; https://doi.org/10.6028/NIST.AI.100-1]

Do-mode investment earns positive return mainly as a bridge, when demand is uncertain, the workflow is changing too quickly to encode safely, or the build effort exceeds about two years of AI-assisted delivery. [inference; source: https://cloud.google.com/resources/content/dora-roi-of-ai-assisted-software-development; https://dora.dev/research/2024/dora-report/; https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-05-13-agent-process-reliability-architecture.md]

Stable, high-frequency, compliance-relevant work should usually migrate out of do-mode and into deterministic system capability because recurring oversight cost compounds while build-mode converts the same need into a lower-maintenance asset. [inference; source: https://doi.org/10.6028/NIST.AI.100-1; https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-05-09-hybrid-architecture-probabilistic-llm-deterministic-governance.md; https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-05-13-agent-process-reliability-architecture.md]
...
3. **Current software-engineering productivity evidence supports a planning multiplier band from about 1.10x at the organizational level to about 1.56x on bounded tasks, with about 1.26x as the strongest central estimate for real deployment economics.** ([inference]; high confidence; source: https://cloud.google.com/resources/content/dora-impact-of-gen-ai-software-development; https://www.microsoft.com/en-us/research/publication/the-effects-of-generative-ai-on-high-skilled-work-evidence-from-three-field-experiments-with-software-developers/; https://arxiv.org/abs/2302.06590)
4. **Using the central 1.26x multiplier, representative three-year build-mode totals are about $97,000 for integration debt, $145,000 for functionality debt, and $193,000 for data-access debt, all materially below the standard $336,000 three-year do-mode cost.** ([inference]; medium confidence; source: https://www.microsoft.com/en-us/research/publication/the-effects-of-generative-ai-on-high-skilled-work-evidence-from-three-field-experiments-with-software-developers/; https://www.bls.gov/oes/2023/may/oes151252.htm; https://www.bls.gov/news.release/ecec.t01.htm; https://cloud.google.com/resources/content/dora-roi-of-ai-assisted-software-development)
5. **Under the same central assumptions, build-mode breaks even against a standard do-mode operating model at about 20.9 delivery-months over three years and about 29.8 delivery-months over five years, which means many stable capability gaps should be closed rather than worked around.** ([inference]; medium confidence; source: https://www.microsoft.com/en-us/research/publication/the-effects-of-generative-ai-on-high-skilled-work-evidence-from-three-field-experiments-with-software-developers/; https://www.bls.gov/oes/2023/may/oes151252.htm; https://www.bls.gov/news.release/ecec.t01.htm; https://cloud.google.com/resources/content/dora-roi-of-ai-assisted-software-development)
6. **Do-mode keeps a positive return profile mainly when the closure effort is unusually large, the demand pattern is intermittent, or the operating design is explicitly temporary while the organization learns the workflow and validates the target state.** ([inference]; medium confidence; source: https://cloud.google.com/resources/content/dora-roi-of-ai-assisted-software-development; https://dora.dev/research/2024/dora-report/; https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-05-13-agent-process-reliability-architecture.md)
7. **For regulated or safety-relevant workflows, the required review and evidence burden shifts the economics further toward build-mode because stricter oversight raises recurring do-mode cost faster than it raises post-build maintenance cost.** ([inference]; medium confidence; source: https://doi.org/10.6028/NIST.AI.100-1; https://nvlpubs.nist.gov/nistpubs/ai/NIST.AI.600-1.pdf; https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-05-09-hybrid-architecture-probabilistic-llm-deterministic-governance.md)
8. **The strategic role of do-mode is therefore best understood as bridge capital for uncertain or rapidly changing work, not as the default steady-state operating model for stable high-frequency system gaps.** ([inference]; medium confidence; source: https://cloud.google.com/resources/content/dora-roi-of-ai-assisted-software-development; https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-05-13-agent-process-reliability-architecture.md; https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-05-09-hybrid-architecture-probabilistic-llm-deterministic-governance.md)
...
| [inference] The defensible productivity band is 1.10x to 1.56x, centered around 1.26x. | https://cloud.google.com/resources/content/dora-impact-of-gen-ai-software-development ; https://www.microsoft.com/en-us/research/publication/the-effects-of-generative-ai-on-high-skilled-work-evidence-from-three-field-experiments-with-software-developers/ ; https://arxiv.org/abs/2302.06590 | high | Uses organization, field, and bounded-task evidence together. |
| [inference] Three-year build totals for integration, functionality, and data-access debt remain below the standard three-year do-mode total. | https://www.microsoft.com/en-us/research/publication/the-effects-of-generative-ai-on-high-skilled-work-evidence-from-three-field-experiments-with-software-developers/ ; https://www.bls.gov/oes/2023/may/oes151252.htm ; https://www.bls.gov/news.release/ecec.t01.htm ; https://cloud.google.com/resources/content/dora-roi-of-ai-assisted-software-development | medium | Relies on explicit archetype effort assumptions. |
| [inference] Standard do-mode breaks even only when closure effort exceeds about 21 delivery-months over three years and about 30 over five years. | https://www.microsoft.com/en-us/research/publication/the-effects-of-generative-ai-on-high-skilled-work-evidence-from-three-field-experiments-with-software-developers/ ; https://www.bls.gov/oes/2023/may/oes151252.htm ; https://www.bls.gov/news.release/ecec.t01.htm ; https://cloud.google.com/resources/content/dora-roi-of-ai-assisted-software-development | medium | Threshold is sensitive to staffing assumption but direction is robust. |
| [inference] Do-mode is strongest as a temporary bridge for uncertain or fast-changing work. | https://cloud.google.com/resources/content/dora-roi-of-ai-assisted-software-development ; https://dora.dev/research/2024/dora-report/ ; https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-05-13-agent-process-reliability-architecture.md | medium | Supported by DORA's J-curve and prior repository architecture findings. |
| [inference] Stricter control environments move the economics further toward build-mode. | https://doi.org/10.6028/NIST.AI.100-1 ; https://nvlpubs.nist.gov/nistpubs/ai/NIST.AI.600-1.pdf ; https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-05-09-hybrid-architecture-probabilistic-llm-deterministic-governance.md | medium | Governance intensity increases recurring do-mode labor more than post-build maintenance. |
| [inference] Stable, high-frequency work should migrate from do-mode into deterministic system capability. | https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-05-13-agent-process-reliability-architecture.md ; https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-05-09-hybrid-architecture-probabilistic-llm-deterministic-governance.md ; https://cloud.google.com/resources/content/dora-roi-of-ai-assisted-software-development | medium | This is the combined economic and control-boundary conclusion. |
...
- [assumption] **Do-mode staffing bands:** lean 0.35 full-time equivalent, standard 0.50, strict 1.00, plus modest platform spend. **Justification:** NIST governance functions, DORA trust findings, and GitHub review mechanics imply non-trivial recurring labor. Source: https://doi.org/10.6028/NIST.AI.100-1 ; https://dora.dev/research/2024/ai-preview/ ; https://docs.github.com/en/copilot/how-tos/copilot-on-github/use-copilot-agents/kick-off-a-task ; https://docs.github.com/en/copilot/how-tos/copilot-on-github/use-copilot-agents/copilot-code-review
- [assumption] **Debt-class effort archetypes:** integration six delivery-months, functionality nine, data access twelve. **Justification:** the item needs generalized debt classes rather than vendor quotes, and the three classes represent increasing complexity and governance burden. Source: https://doi.org/10.6028/NIST.AI.100-1 ; https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-05-13-agent-process-reliability-architecture.md
...
The strongest primary evidence in this item is structural rather than anecdotal: model prices are explicit, governance functions are explicit, and software-engineering productivity studies provide bounded multiplier ranges. [fact; source: https://www.anthropic.com/pricing#api; https://cloud.google.com/gemini-enterprise-agent-platform/generative-ai/pricing; https://doi.org/10.6028/NIST.AI.100-1; https://www.microsoft.com/en-us/research/publication/the-effects-of-generative-ai-on-high-skilled-work-evidence-from-three-field-experiments-with-software-developers/; https://arxiv.org/abs/2302.06590]

That evidence makes the central trade-off clear: do-mode looks cheap if the analyst compares only token cost to developer salary, but it looks expensive once the recurring human review, governance, and failure-recovery loop is priced in. [inference; source: https://dora.dev/research/2024/ai-preview/; https://docs.github.com/en/copilot/how-tos/copilot-on-github/use-copilot-agents/kick-off-a-task; https://docs.github.com/en/copilot/how-tos/copilot-on-github/use-copilot-agents/copilot-code-review; https://doi.org/10.6028/NIST.AI.100-1]

The build-mode side is more assumption-sensitive, so the analysis keeps the logic transparent: loaded labor, productivity multiplier, debt-class effort, and maintenance rate are all visible, and changing them shifts the breakeven month threshold rather than reversing the direction for common stable gaps. [inference; source: https://www.bls.gov/oes/2023/may/oes151252.htm; https://www.bls.gov/news.release/ecec.t01.htm; https://www.microsoft.com/en-us/research/publication/the-effects-of-generative-ai-on-high-skilled-work-evidence-from-three-field-experiments-with-software-developers/; https://cloud.google.com/resources/content/dora-roi-of-ai-assisted-software-development]
...
- Review result: pass
- Acronym audit: Artificial Intelligence (AI), Large Language Model (LLM), National Institute of Standards and Technology (NIST), Generative Artificial Intelligence (GenAI), Artificial Intelligence Risk Management Framework (AI RMF), Large Language Models (LLMs), pull request (PR)
- Domain-term audit: do-mode, build-mode, integration debt, functionality debt, data-access debt defined in question, scope, or assumptions
- Claim audit: visible claims in Research Skill Output labeled as fact, inference, or assumption
- Findings parity audit: §6 Synthesis and Findings aligned on claims, sources, and confidence
- Remaining uncertainty: archetype effort and staffing assumptions drive the exact threshold values
