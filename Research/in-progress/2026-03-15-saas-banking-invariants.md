...
- **[fact]** Software as a Service (SaaS) is a cloud delivery model in which the provider runs the application and the customer uses it through a service relationship rather than owning a perpetual installed product. Source: tertiary [x] https://en.wikipedia.org/wiki/Software_as_a_service
- **[fact]** Total Cost of Ownership (TCO) is the lifecycle estimate of direct and indirect costs to acquire, configure, operate, maintain, upgrade, and retire a software product or service. Source: tertiary [x] https://en.wikipedia.org/wiki/Total_cost_of_ownership; secondary [x] https://www.cio.com/article/242681/calculating-the-total-cost-of-ownership-for-enterprise-software.html
- **[inference]** For this item, an **invariant** is treated as a capability or property that the platform supplies without the customer having to invent it, even if the customer still must configure, integrate, govern, or operationalize it.
...
- **[fact]** The prior completed item `2026-03-13-financial-forecasting-it-run-costs.md` independently concluded that responsible IT cost models must include licensing, support, labour, uncertainty, and lifecycle obligations rather than just acquisition cost. Source: prior-art [x] `Research/completed/2026-03-13-financial-forecasting-it-run-costs.md`
...
- **[fact]** The strongest evidence for what is truly invariant comes from official vendor architecture, implementation, and platform documentation rather than from product-marketing taglines alone.
- **[fact]** Across Salesforce, nCino, Temenos, and Thought Machine, the common pattern is prebuilt domain primitives plus shared platform operations, not turnkey institution-specific outcomes.
- **[inference]** Therefore the correct analytical boundary is: packaged models/workflow engines/integration surfaces/security envelopes are invariant; customer-specific process fit, data quality, and operational adoption are not.
- **[fact]** Full lifecycle cost evidence from CIO and from nCino's own implementation process independently confirms that migration, testing, training, and administration are structural, recurring cost categories.
- **[inference]** The economic comparison must therefore be made at lifecycle level, not at license-price or initial-build-effort level.
- **[fact]** AI evidence from GitHub and Deloitte is specific enough to support claims about reduced coding and testing effort, but not strong enough to claim that AI removes domain, integration, or governance costs.
- **[inference]** The AI-era conclusion should be framed as a threshold shift, not as a universal reversal of buy-versus-build economics.

### Section 4 Consistency Check

- **[fact]** A tension appeared between vendor claims of broad out-of-the-box value and implementation evidence showing large required customer effort. This was resolved by distinguishing packaged capability availability from implementation-independent business outcome.
- **[fact]** A second tension appeared between "banks mostly buy" evidence and "build or buy is a false binary" evidence. This was resolved by treating the market pattern as layered: banks buy broad platform capability and selectively build differentiating layers.
- **[fact]** Confidence levels were calibrated downward where public evidence was marketing-led or where proprietary analyst material was inaccessible.
- **[fact]** No unsupported claim was left in the synthesis without either a source, an inference label, or an explicit gap note.
...
|---|---|---|---|
| [fact] FSC and nCino provide reusable domain models and workflow scaffolding rather than turnkey institution-specific outcomes | [x] Salesforce FSC User Guide; [x] Salesforce FSC Admin Guide; [x] Trailhead FSC Basics; [x] nCino platform pages | High | Cross-vendor official documentation aligns; extends `2026-03-08-servicenow-platform-strategy.md` |
| [fact] Customer effort remains structurally necessary for integration, mapping, testing, training, and administration | [x] nCino implementation page; [x] CIO TCO article; [x] Salesforce admin documentation | High | Strongest evidence comes from vendor implementation docs themselves |
| [fact] Salesforce invariants are mainly shared platform operations plus domain extensions | [x] Salesforce platform overview; [x] Salesforce trust docs; [x] Salesforce multitenancy explainer | High | Distinguishes platform primitives from FSC domain package |
| [fact] nCino invariants are packaged banking workflows plus banking-oriented integration patterns | [x] nCino platform overview; [x] nCino commercial banking page; [x] nCino developer portal; [x] nCino Integration Gateway listing | Medium | Official evidence is strong on capability shape, weaker on economic quantification |
| [fact] Modern banking platform invariants are industry-wide, not unique to FSC or nCino | [x] Temenos core banking page; [x] Thought Machine Vault Core; [x] nCino platform overview | High | Independent vendor pages show the same primitive pattern |
| [inference] Pre-AI, buy/rent usually beat full bespoke build for broad banking capability | [x] American Bankers Association (ABA) core platforms survey; [x] PwC cloud banking trends; [x] Seerene build-or-buy analysis | Medium | Public evidence supports direction of travel, but analyst detail is partly paywalled |
| [fact] Honest banking platform TCO must include hidden lifecycle categories beyond sticker price | [x] Chief Information Officer (CIO) TCO article; [x] nCino implementation page; [x] `2026-03-13-financial-forecasting-it-run-costs.md` | High | Strong cross-source agreement |
| [inference] AI narrows bespoke cost disadvantage but does not eliminate non-code transformation costs | [x] GitHub Copilot economic-impact research; [x] Deloitte future of software engineering in banks; [x] Chief Information Officer (CIO) TCO article | Medium | Good evidence for coding/testing gains; remaining lifecycle-cost conclusion is partly inferential |
...
Enterprise banking Software as a Service (SaaS) platforms do not provide end-to-end banking outcomes as true invariants; their real invariants are vendor-maintained domain models, workflow scaffolding, integration surfaces, security/compliance envelopes, and shared operating infrastructure. Salesforce Financial Services Cloud (FSC) and nCino both provide substantial reusable banking functionality, but their own official documentation shows that core integration, data mapping, configuration, testing, training, and administration remain customer responsibilities. Before Artificial Intelligence (AI)-assisted development, that still made buying or renting broad banking capability economically attractive for most institutions because vendors amortized complex domain and platform investment across many customers. In the AI-assisted era, bespoke software becomes materially more competitive for differentiated workflow and experience layers because coding, explanation, and test effort fall, but AI does not remove migration, integration, governance, or regulatory assurance work. The build-versus-buy threshold therefore shifts toward a layered architecture: buy platform primitives and commodity banking capability; build where differentiation justifies the remaining lifecycle cost.

### Key Findings

1. **Confidence: high.** Salesforce Financial Services Cloud (FSC) and nCino both provide real invariant value at the level of reusable financial-services data models, packaged workflow shells, open integration surfaces, and vendor-operated platform services, rather than at the level of institution-specific end-to-end banking outcomes.

2. **Confidence: high.** The vendors' own implementation materials show that integration with core systems, data mapping and migration, configuration, testing, training, and platform administration are normal and unavoidable parts of adoption, which means customer effort remains a structural part of value realization.

3. **Confidence: high.** Salesforce's strongest invariants come from the Lightning Platform and trust model - multitenant infrastructure, shared security/compliance documentation, shared tooling, and centrally managed operations - while Financial Services Cloud adds domain-specific objects and process scaffolding on top of those primitives.

4. **Confidence: medium.** nCino's strongest invariants are packaged banking workflows for onboarding, account opening, lending, and portfolio management plus an implementation and integration model tuned to banking institutions, but these remain dependent on local core connectivity and operating-model fit.

5. **Confidence: high.** Comparable vendors such as Temenos and Thought Machine market the same underlying invariant pattern - configurable domain engines, vendor-maintained core capabilities, and exposed integration surfaces - showing that these invariants are industry-wide characteristics of modern banking platforms rather than unique properties of Salesforce Financial Services Cloud or nCino.

6. **Confidence: medium.** Before Artificial Intelligence (AI)-assisted development, buying or renting broad banking platform capability was economically attractive for most banks because vendor products bundled proven domain functionality, ecosystem leverage, and ongoing platform investment that many institutions would struggle to reproduce internally at acceptable speed and risk.

7. **Confidence: high.** Full lifecycle Total Cost of Ownership (TCO) for banking Software as a Service (SaaS) platforms must include not only subscription or license spend but also integration, migration, security review, process redesign, regression testing, training, administration, and retirement or export work, because those categories recur across both vendor guidance and independent TCO literature.

8. **Confidence: medium.** Artificial Intelligence (AI)-assisted development narrows the economic gap for bespoke software by reducing coding, legacy-code explanation, and test-authoring effort, but it does not proportionally reduce data migration, integration, governance, adoption, or regulatory assurance costs, so it shifts the threshold rather than eliminating the case for buying platforms.

### Evidence Map

| Claim | Source | Confidence | Notes |
|---|---|---|---|
| FSC and nCino provide reusable domain models and workflow scaffolding rather than turnkey institution-specific outcomes | Salesforce FSC User Guide; Salesforce FSC Admin Guide; Trailhead FSC Basics; nCino platform pages | high | Official documentation aligned across vendors |
| Customer effort remains necessary for integration, mapping, testing, training, and administration | nCino implementation page; CIO TCO article; Salesforce admin documentation | high | Explicitly documented in implementation and lifecycle material |
| Salesforce invariants are mainly shared platform operations plus domain extensions | Salesforce platform overview; Salesforce trust docs; Salesforce multitenancy explainer | high | Strong separation between platform primitives and FSC-specific layer |
| nCino invariants are packaged banking workflows plus banking-oriented integration patterns | nCino platform overview; nCino commercial banking page; nCino developer portal; nCino Integration Gateway listing | medium | Strong official evidence on capability shape; less precise on economics |
| Modern banking platform invariants are industry-wide, not unique to FSC or nCino | Temenos core banking page; Thought Machine Vault Core; nCino platform overview | high | Cross-vendor validation |
| Pre-AI, buy/rent usually beat full bespoke build for broad banking capability | ABA core platforms survey; PwC cloud banking trends; Seerene build-or-buy analysis | medium | Direction strongly supported; exact magnitude less certain |
| Honest banking platform TCO must include hidden lifecycle categories beyond sticker price | CIO TCO article; nCino implementation page; `2026-03-13-financial-forecasting-it-run-costs.md` | high | Strong cross-source agreement |
| AI narrows bespoke cost disadvantage but does not eliminate non-code transformation costs | GitHub Copilot economic-impact research; Deloitte future of software engineering in banks; CIO TCO article | medium | Development productivity well supported; transformation-cost conclusion partly inferential |
...
The evidence is strongest where the question is reframed from "what feature exists?" to "what kind of thing is actually invariant?" Official Salesforce and nCino materials support a clear distinction between packaged domain capability and institution-specific execution. Both vendors genuinely reduce invention effort by supplying data models, workflows, compliance-aware process shells, and ecosystem integrations. Both also openly document the work still left to the customer. That combination is the key to the item.

The cross-vendor check prevents overstating Salesforce Financial Services Cloud (FSC) or nCino as uniquely privileged. Temenos and Thought Machine expose the same pattern from another angle: configurable domain engines, exposed integration surfaces, and vendor-maintained platform mechanics. That makes the category-level conclusion more robust than a narrow product comparison.

The economic comparison is most persuasive when separated into two eras. Before Artificial Intelligence (AI)-assisted development, the build side paid a large penalty in coding throughput, legacy comprehension, test generation, and platform plumbing. After AI, those penalties shrink, but the non-code work remains: migration, integration, process design, user adoption, governance, and assurance. That means AI improves the economics of bespoke differentiation layers far more than it improves the economics of rebuilding an entire commodity banking platform from scratch.

### Risks, Gaps, and Uncertainties

- Gartner, Forrester, and directly fetched McKinsey banking articles were not accessible in this session, so the evidence base leans more heavily on official vendor documentation and accessible secondary banking-industry commentary than would be ideal.
- Public sources do not expose enough pricing detail to calculate a generally valid numeric Total Cost of Ownership (TCO) crossover point between named vendor platforms and bespoke systems.
- nCino's original `/products` web address returned status code 429, so the research used other official nCino pages that were accessible instead.
- Vendor case material is structurally biased toward benefit claims; confidence is therefore lower on any claim that relies primarily on vendor marketing rather than on implementation mechanics or independent lifecycle-cost evidence.
- The exact effect of Artificial Intelligence (AI) on regulated-software assurance cost remains uncertain; the evidence is stronger on development productivity than on audit or regulatory-review compression.
