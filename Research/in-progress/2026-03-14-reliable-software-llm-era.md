...
[inference] The Informal Systems proposal addresses the identified reliability problem — the break in the "understand-while-coding" trust mechanism. [inference] The architecture (English → Quint → Implementation) is internally consistent because it preserves human ownership at the spec layer while delegating implementation translation to LLMs. [inference] The critical question is adoption friction. Quint requires teams to learn a new language (however friendly), maintain an additional artefact (the spec), and change their development process. The quint-llm-kit reduces this friction by making LLMs generate the spec from English and by providing MCP tooling that integrates Quint into the LLM agent's workflow — but it is currently research-grade.
