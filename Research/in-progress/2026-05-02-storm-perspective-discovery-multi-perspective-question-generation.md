...
The best minimum-viable §0.5 design for this repository is therefore an additive four-perspective prompt that emits one seed question per non-overlapping lens and then hands those questions to §1 Question Decomposition unchanged. [inference; source: https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/system-prompts; https://www.anthropic.com/engineering/building-effective-agents; https://github.com/stanford-oval/storm/blob/fb951af7744dab086e34962e9bc6fe878e145f83/knowledge_storm/storm_wiki/modules/knowledge_curation.py]

**Key findings:**

1. **STORM discovers perspectives by surveying related Wikipedia pages, extracting their titles and tables of contents, and then prompting the model to invent editor personas that each represent a different perspective, role, or affiliation before any question decomposition begins.** ([fact]; high confidence; source: https://aclanthology.org/2024.naacl-long.347/; https://github.com/stanford-oval/storm/blob/fb951af7744dab086e34962e9bc6fe878e145f83/knowledge_storm/storm_wiki/modules/persona_generator.py)
2. **The released implementation always prepends a `Basic fact writer` persona, which means STORM explicitly combines one broad factual lens with a small set of topic-specific lenses instead of relying only on specialist roles.** ([fact]; high confidence; source: https://github.com/stanford-oval/storm/blob/fb951af7744dab086e34962e9bc6fe878e145f83/knowledge_storm/storm_wiki/modules/persona_generator.py)
3. **STORM does not publish an explicit diversity rubric over disciplinary, cultural, temporal, or stakeholder categories, so its diversity mechanism is implicit and retrieval-primed rather than a formal coverage algorithm with declared quotas.** ([fact]; high confidence; source: https://aclanthology.org/2024.naacl-long.347/; https://github.com/stanford-oval/storm/blob/fb951af7744dab086e34962e9bc6fe878e145f83/knowledge_storm/storm_wiki/modules/persona_generator.py)
4. **The paper's +10% `broad in coverage` result should not be attributed to perspective discovery alone, because that number compares full STORM against an outline-driven retrieval-augmented generation baseline and the ablations isolate a smaller persona-specific effect.** ([fact]; high confidence; source: https://aclanthology.org/2024.naacl-long.347/)
5. **The ablation results show that removing perspective conditioning roughly halves the number of unique references collected and lowers entity-level outline recall, while removing simulated conversation hurts performance even more, so the conversation loop appears to contribute more than persona seeding to overall question quality.** ([fact]; high confidence; source: https://aclanthology.org/2024.naacl-long.347/)
...
7. **The minimum-viable prompt for this repository should emit exactly four non-overlapping perspectives, `basic facts`, `mechanism or implementation`, `stakeholder or decision impact`, and `failure mode or critic`, with one seed question per perspective, because that preserves STORM's broad-facts-plus-specialist-lenses pattern while staying simple enough for a single-agent workflow.** ([inference]; medium confidence; source: https://www.anthropic.com/engineering/building-effective-agents; https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/system-prompts; https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-03-10-adversarial-agents-shared-goals-multi-perspective.md)
8. **The proposed §0.5 step is additive rather than breaking, because §1 can decompose the emitted seed questions exactly as it already decomposes the original research question, leaving §§2-§7 unchanged.** ([inference]; medium confidence; source: https://github.com/davidamitchell/Research/blob/main/BACKLOG.md; https://aclanthology.org/2024.naacl-long.347/)

**Evidence map:**

| Claim | Source | Confidence | Notes |
|---|---|---|---|
| [fact] STORM surveys related Wikipedia pages, extracts tables of contents, and generates personas before asking questions. | https://aclanthology.org/2024.naacl-long.347/; https://github.com/stanford-oval/storm/blob/fb951af7744dab086e34962e9bc6fe878e145f83/knowledge_storm/storm_wiki/modules/persona_generator.py | high | Paper plus code |
| [fact] STORM prepends a `Basic fact writer` persona to topic-specific personas. | https://github.com/stanford-oval/storm/blob/fb951af7744dab086e34962e9bc6fe878e145f83/knowledge_storm/storm_wiki/modules/persona_generator.py | high | Code-backed |
| [fact] Diversity criteria are implicit `perspective, role, or affiliation`, not a fixed rubric. | https://aclanthology.org/2024.naacl-long.347/; https://github.com/stanford-oval/storm/blob/fb951af7744dab086e34962e9bc6fe878e145f83/knowledge_storm/storm_wiki/modules/persona_generator.py | high | Prompt-backed |
| [fact] The +10% breadth claim belongs to end-to-end STORM versus outline-driven retrieval-augmented generation, not to the persona step alone. | https://aclanthology.org/2024.naacl-long.347/ | high | Human-eval claim |
| [fact] Ablations show perspective discovery matters, but conversation matters more, especially for unique references and entity recall. | https://aclanthology.org/2024.naacl-long.347/ | high | Table 3 plus Table 5 |
...
| [inference] A four-slot prompt is the closest minimum-viable single-agent analogue because it preserves STORM's broad-facts-plus-specialist-lenses pattern. | https://www.anthropic.com/engineering/building-effective-agents; https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/system-prompts; https://github.com/davidamitchell/Research/blob/main/Research/completed/2026-03-10-adversarial-agents-shared-goals-multi-perspective.md | medium | Design synthesis |
| [inference] The §0.5 step can be inserted without downstream workflow changes because it only emits seed questions for §1. | https://github.com/davidamitchell/Research/blob/main/BACKLOG.md; https://aclanthology.org/2024.naacl-long.347/ | medium | Integration judgment |
...
STORM's released code and paper align on a narrow but actionable interpretation of `perspective discovery`: it is a prompt-driven persona seeding stage that happens before question asking, not a formal optimisation pass over declared diversity dimensions. [fact; source: https://aclanthology.org/2024.naacl-long.347/; https://github.com/stanford-oval/storm/blob/fb951af7744dab086e34962e9bc6fe878e145f83/knowledge_storm/storm_wiki/modules/persona_generator.py]
...
- [fact] Every factual claim retained in the synthesis and findings sections is bound to an external URL or a URL-backed prior completed item. Source: https://aclanthology.org/2024.naacl-long.347/; https://github.com/stanford-oval/storm/blob/fb951af7744dab086e34962e9bc6fe878e145f83/README.md; https://github.com/stanford-oval/storm/blob/fb951af7744dab086e34962e9bc6fe878e145f83/knowledge_storm/storm_wiki/modules/persona_generator.py; https://github.com/stanford-oval/storm/blob/fb951af7744dab086e34962e9bc6fe878e145f83/knowledge_storm/storm_wiki/modules/knowledge_curation.py; https://arxiv.org/abs/2203.11171; https://www.debono.com/six-thinking-hats-summary; https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/system-prompts; https://www.anthropic.com/engineering/building-effective-agents
- [fact] The strongest claims, STORM's mechanics, the evaluation metrics, and the ablation values, are supported by the primary STORM paper plus released implementation. Source: https://aclanthology.org/2024.naacl-long.347/; https://github.com/stanford-oval/storm/blob/fb951af7744dab086e34962e9bc6fe878e145f83/knowledge_storm/storm_wiki/modules/persona_generator.py; https://github.com/stanford-oval/storm/blob/fb951af7744dab086e34962e9bc6fe878e145f83/knowledge_storm/storm_wiki/modules/knowledge_curation.py
- [inference] Overall confidence remains medium because the recommended §0.5 prompt block is a design synthesis for this repository and has not yet been benchmarked locally against question-coverage outcomes. Source: https://aclanthology.org/2024.naacl-long.347/; https://www.anthropic.com/engineering/building-effective-agents
