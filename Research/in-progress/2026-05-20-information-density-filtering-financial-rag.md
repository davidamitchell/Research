...
The strongest evidence for this design is that financial-document Retrieval-Augmented Generation (RAG) systems remain much weaker on cross-document synthesis than on factual extraction, so duplicated or low-information context increases risk exactly where these workflows are already fragile. [fact; source: https://aclanthology.org/2025.finnlp-2.9.pdf]
...
6. **Financial-document benchmarks show that current Retrieval-Augmented Generation (RAG) architectures remain much weaker on multi-document synthesis than on single-document extraction, so duplicate and low-signal context should be treated as a material accuracy risk rather than as a minor efficiency issue.** ([fact]; high confidence; source: https://aclanthology.org/2025.finnlp-2.9.pdf)
...
9. **Anti-Money Laundering workflows already suffer from false positives, unclear data mapping, and synchronization gaps between monitoring and underlying business activity, so corpus cleanup should be justified partly as an investigator-workload and consistency control, not only as a model-quality optimization.** ([fact]; medium confidence; source: https://www.deloitte.com/ch/en/Industries/financial-services/blogs/aml-transaction-monitoring.html)
...
| [fact] Financial Retrieval-Augmented Generation (RAG) performance drops sharply on cross-document synthesis compared with factual extraction. | https://aclanthology.org/2025.finnlp-2.9.pdf | high | 0.91 versus 0.44 task gap |
...
| [fact] Anti-Money Laundering investigators already face false positives, unclear mapping, and synchronization failures. | https://www.deloitte.com/ch/en/Industries/financial-services/blogs/aml-transaction-monitoring.html | medium | Workflow-burden evidence |
...
Financial evidence matters because the domain penalty for bad retrieval context is asymmetric: FinDoc-RAG shows the largest weakness on cross-document synthesis, which is precisely the setting where duplicate passages, superseded copies, and wrapper text can crowd out the small number of passages that actually settle a credit-risk or Anti-Money Laundering judgment. [inference; source: https://aclanthology.org/2025.finnlp-2.9.pdf]
