...
- [fact; source: https://opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-events/; https://docs.aws.amazon.com/bedrock/latest/userguide/trace-events.html] The strongest direct evidence concerns what modern trace systems can already record: prompts, inference parameters, tool activity, retrieval activity, conversation identifiers, and caller chains.
- [fact; source: https://www.w3.org/TR/prov-overview/; https://martinfowler.com/eaaDev/EventSourcing.html] The strongest formal analogies concern provenance modelling and event logs, because both disciplines are explicitly designed for reconstruction, derivation, and temporal reasoning.
...
- [fact; source: https://docs.aws.amazon.com/bedrock/latest/userguide/model-invocation-logging.html; https://docs.aws.amazon.com/bedrock/latest/userguide/trace-events.html] AWS Bedrock exposes both per-step agent traces and broader invocation logging, which is consistent with the claim that one telemetry stream is insufficient for a full runtime AIBOM.
...
- A runtime-observed AIBOM is best generated as an event-sourced provenance graph plus decision-point state snapshots, and it will diverge materially from the declared design-time AIBOM on almost every non-trivial run. [inference; source: https://www.w3.org/TR/prov-overview/; https://martinfowler.com/eaaDev/EventSourcing.html; https://opentelemetry.io/docs/concepts/signals/traces/]
...
- AWS Bedrock is useful because it demonstrates that production agent traces already include the run objects this theory needs, including caller chain, prompt text, orchestration step types, rationale, observations, and failure traces, yet it also shows that runtime evidence remains fragmented across multiple log modes. [fact; source: https://docs.aws.amazon.com/bedrock/latest/userguide/trace-events.html; https://docs.aws.amazon.com/bedrock/latest/userguide/model-invocation-logging.html]
