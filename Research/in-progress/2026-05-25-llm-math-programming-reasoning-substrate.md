...
- 2b. What are the benchmark results from the MATH dataset and GSM8K (Grade School Math 8,000)?
...
**Discarded claims:**
- "Formal symbolic language alone aligns with LLM reasoning" -- this is not established by current evidence. Training data volume and external verifiability are the better-supported explanations.
...
2. CoT prompting disproportionately benefits mathematical reasoning tasks: Wei et al. (2022) reported a jump from 17.9% to 57.1% on GSM8K for GPT-3 175B, with the benefit scaling with model size and being most pronounced in multi-step arithmetic and commonsense reasoning. (high confidence; source: https://arxiv.org/abs/2201.11903)

3. External verifiability is the operationally strongest mechanism explaining why code generation is a high-confidence LLM deployment surface: compilers, type checkers, test suites, and formal proof assistants compute independent, reproducible verdicts over code artifacts before deployment. (high confidence; source: https://davidamitchell.github.io/Research/research/2026-04-26-llm-verifiability-asymmetry-code-world-action.html; https://arxiv.org/abs/2107.03374)
...
7. Training data domain distribution is a confounding factor: Minerva's 50.3% on MATH compared to PaLM-540B's 8.8% on the same benchmark, with the only difference being domain-specific pre-training on mathematical content, shows that training data distribution explains more variance than model scale alone. (high confidence; source: https://arxiv.org/abs/2206.14858)

8. CoT prompting provides minimal benefit for small models (below approximately 100 billion parameters), indicating that the reasoning advantage in formal symbolic domains emerges from model scale rather than from formal structure recognition per se. (medium confidence; source: https://arxiv.org/abs/2201.11903)
...
```text
review_result: pass
acronym_audit: LLM (Large Language Model) expanded on first use in Research Question; CoT (chain-of-thought) expanded in §1; GSM8K expanded in §2.3; IMO expanded in §2.5; PLDI expanded in §2.1; LMQL expanded in §2.1; RL expanded in §2.5; MBPP expanded in §1 sub-question header
...
2. **CoT prompting disproportionately benefits mathematical reasoning tasks, raising GPT-3 175B performance on GSM8K from 17.9% to 57.1%, with the benefit scaling with model size and being most pronounced in multi-step arithmetic and commonsense reasoning.** (high confidence; source: https://arxiv.org/abs/2201.11903)

3. **External verifiability is the operationally strongest mechanism explaining why code generation is a high-confidence LLM deployment surface: compilers, type checkers, test suites, and formal proof assistants compute independent, reproducible verdicts over code artifacts before deployment, providing a correction loop absent in natural language generation.** (high confidence; source: https://davidamitchell.github.io/Research/research/2026-04-26-llm-verifiability-asymmetry-code-world-action.html; https://arxiv.org/abs/2107.03374)
...
7. **Training data domain distribution is a confounding factor: Minerva 62B achieves 50.3% on the MATH dataset compared to PaLM-540B's 8.8%, with domain-specific mathematical pre-training -- not model scale -- explaining the performance difference.** (high confidence; source: https://arxiv.org/abs/2206.14858)
...
The external verifiability mechanism is the operationally most important: it is the mechanism that allows code and formal math to be error-corrected by external tools, and it is the mechanism behind AlphaProof's IMO performance. Prior repository work (verifiability asymmetry item) established this as the principled deployment distinction between code and world actions. This item strengthens that finding: without Lean verification, AlphaProof-level IMO performance is not achievable; the formal verifier is the enabling mechanism, not symbolic language structure alone.
