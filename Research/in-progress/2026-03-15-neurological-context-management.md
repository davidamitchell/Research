...
- [fact] The research question is: how do human brains store, compress, retrieve, and dynamically layer values, goals, rules, current state, and immediate task context during decision-making, and which AI context-management design principles are supportable from that evidence.
- [fact] The scope includes working memory limits, prefrontal control, predictive processing, attention and inhibition, episodic and semantic memory interaction, schema formation, and dual-process reasoning, while excluding clinical pathology and low-level biochemical circuitry.
- [fact] The constraints are to rely on public sources with URL-level citations and to separate [fact], [inference], and [assumption] claims throughout the note.
- [fact] Prior-work cross-reference found three directly relevant completed items before investigation: `2026-02-28-predictive-processing-active-inference.md` for predictive-processing mechanics, `2026-03-17-ai-memory-systems-rag-neuroscience.md` for memory-layer analogies, and `2026-03-15-context-layers-aligned-decisions-synthesis.md` for the downstream architectural target this item is intended to unblock.
- [inference] The required output format is a knowledge note whose §6 Synthesis establishes every claim that later appears in `## Findings`.
...
- [fact] Baddeley proposed the episodic buffer as a fourth working-memory component because the original model did not adequately explain multimodal binding or communication with long-term memory. He described the episodic buffer as a limited-capacity store that binds information from subsidiary systems and long-term memory into a unitary episodic representation. [Source class: secondary review; https://pubmed.ncbi.nlm.nih.gov/11058819/]
...
- [inference] Human contextual reasoning does not keep many raw facts simultaneously active; it keeps a few bound chunks that already encode relationships, relevance, and prior structure.
...
- [inference] The prefrontal cortex is not just a storehouse; it acts as a gating and arbitration layer that decides which contextual chunk gets maintained, strengthened, or suppressed at a given moment.
...
- [inference] Relevance filtering in the brain combines positive prioritisation of candidate context with active de-prioritisation of already-checked or low-value regions of the search space.
...
- [inference] Brain-like context management needs at least two representational layers: episode-level traces that preserve detail and provenance, and schema-level abstractions that compress repeated structure for rapid use.
...
- [fact] The repository's completed predictive-processing item concluded that the core mechanistic claim of hierarchical prediction and upward propagation of prediction error is well supported, even though the grand-unification scope of the Free Energy Principle remains contested. [Source class: prior completed repository item; `Research/completed/2026-02-28-predictive-processing-active-inference.md`]
- [inference] For contextual reasoning, predictive processing implies that higher-order goals and schemas do not merely sit alongside incoming evidence; they actively shape what counts as salient error and therefore what gets computational priority.
- [inference] A context system built on this principle should privilege deltas, surprises, and conflicts against current goals instead of continually reloading all stable background material.
...
- [inference] Human contextual reasoning appears to rely on a division of labour: schema-driven, low-cost contextual guesses handle most routine cases, while deliberate control escalates when novelty, conflict, or ambiguity exceeds those heuristics.

#### G. Source integration and design implications

- [fact] The completed AI-memory systems item in this repository found that durable AI memory architectures benefit from separating core memory, recall memory, and archival memory rather than using one undifferentiated store. [Source class: prior completed repository item; `Research/completed/2026-03-17-ai-memory-systems-rag-neuroscience.md`]
- [fact] The completed aligned-decision-making synthesis item concluded that organisational decision quality depends on layered context with different authority levels, update rates, and retrieval strategies. [Source class: prior completed repository item; `Research/completed/2026-03-15-context-layers-aligned-decisions-synthesis.md`]
- [inference] The neuroscience evidence strengthens those prior architectural conclusions by adding a stronger rationale for limited active context, explicit gating, and separate episodic-versus-schema memory layers.
- [assumption] Mapping neural mechanisms to AI architecture is an analogy at the level of functional constraints and design principles, not a claim that digital systems should literally reproduce biological circuitry. **Justification:** The item explicitly asks for AI design principles, but the cited evidence is neuroscience rather than engineering validation.
- [assumption] The dual-process mapping to AI fast-path versus deliberate-path control is a useful explanatory analogy, not a settled neuroscientific one-to-one correspondence. **Justification:** the review evidence supports the distinction in cognition, but engineering implementations can only approximate it indirectly.

### §3 Reasoning

- [fact] The strongest evidence in this note concerns hard capacity limits, prefrontal goal maintenance, attention-working-memory coupling, and hippocampal-prefrontal cooperation. Those points are each supported by multiple independent review sources. [Sources: https://pmc.ncbi.nlm.nih.gov/articles/PMC2864034/ ; https://pubmed.ncbi.nlm.nih.gov/11283309/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC5447931/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC6689265/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC3789138/]
...
- [assumption] The most defensible AI analogy is architectural rather than cognitive-philosophical: the note supports claims about memory structure, routing, and escalation logic, but not claims that AI systems thereby become human-like minds. **Justification:** the cited evidence constrains memory, control, and filtering functions, but does not test consciousness or phenomenology.
...
4. [inference] [confidence: medium] Predictive-processing evidence indicates that higher-order goals and schemas act as priors that shape what counts as salient error, so context assembly should focus on exceptions, mismatches, and surprises relative to current goals instead of repeatedly injecting unchanged background knowledge. [Sources: https://doi.org/10.1038/nrn2787 ; `Research/completed/2026-02-28-predictive-processing-active-inference.md`]
...
| [inference] Stable higher-order priors should stay resident while deltas and conflicts get surfaced. | https://doi.org/10.1038/nrn2787 ; `Research/completed/2026-02-28-predictive-processing-active-inference.md` | medium | Strong at the mechanistic level; weaker when translated into engineering architecture. |
...
| [inference] A layered architecture with core priors, active task state, and on-demand retrieval best matches the full evidence set. | https://pmc.ncbi.nlm.nih.gov/articles/PMC2864034/ ; https://pubmed.ncbi.nlm.nih.gov/11283309/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC3789138/ ; `Research/completed/2026-03-15-context-layers-aligned-decisions-synthesis.md` | high | Integrates the strongest convergent evidence and the downstream synthesis target. |

**Assumptions:**

- [assumption] AI architectures can validly borrow functional constraints from neuroscience without needing neuron-level biological fidelity. **Justification:** the item asks for design principles, and the cited evidence is strongest at the level of control problems and memory structure.
- [assumption] Fast-path versus slow-path reasoning in AI is only analogous to human dual-process accounts. **Justification:** the evidence supports the human distinction, but digital systems will implement it differently.
...
- [fact] Every claim-bearing section now labels statements as [fact], [inference], or [assumption], and each [fact] has a URL-level source or an explicitly cited prior repository item.
- [fact] The synthesis claims are all supported by the investigation sections and are repeated, not extended, in `## Findings`.
- [fact] Acronym first-use expansions were checked for Artificial Intelligence (AI), Retrieval-Augmented Generation (RAG), prefrontal cortex (PFC), and Free Energy Principle (FEP).
...
[inference] Human contextual reasoning depends on a layered control system that keeps only a few compressed chunks active, uses prefrontal control to decide which chunks matter now, and relies on hippocampal-prefrontal interactions to turn repeated episodes into reusable schemas. [Sources: https://pmc.ncbi.nlm.nih.gov/articles/PMC2864034/ ; https://pubmed.ncbi.nlm.nih.gov/11283309/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC3789138/]

[fact] The evidence does not support a brain that holds all relevant context in raw form at once; it supports severe active-capacity limits, multimodal binding, top-down attention, and schema-based compression. [Sources: https://pubmed.ncbi.nlm.nih.gov/11058819/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC2864034/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC6689265/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC3789138/]

[inference] The most defensible AI design response is a layered context architecture with always-resident high-authority goals, explicit salience and gating control, separate episodic and schema memory tiers, and escalation from fast heuristics to slower deliberation when conflict or novelty rises. [Sources: https://pubmed.ncbi.nlm.nih.gov/11283309/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC3789138/ ; https://www.frontiersin.org/journals/psychology/articles/10.3389/fpsyg.2022.805386/full ; `Research/completed/2026-03-15-context-layers-aligned-decisions-synthesis.md`]

[inference] This conclusion is strongest as an architectural constraint set, not as a claim that AI should literally copy biological circuitry. [Sources: https://pubmed.ncbi.nlm.nih.gov/11283309/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC3789138/ ; https://www.frontiersin.org/journals/psychology/articles/10.3389/fpsyg.2022.805386/full]

### Key Findings

1. [inference] [confidence: high] Human working memory imposes a hard active-context bottleneck of roughly 3 to 5 meaningful chunks, which means any system that expects high-quality contextual reasoning must compress evidence into bound units instead of keeping a large flat pool of raw context simultaneously active. [Sources: https://pmc.ncbi.nlm.nih.gov/articles/PMC2864034/ ; https://pubmed.ncbi.nlm.nih.gov/11058819/]
2. [inference] [confidence: high] The prefrontal cortex behaves as a control-and-gating layer that maintains goals and biases downstream processing, so AI context systems need an explicit controller that allocates scarce active-context budget rather than relying on retrieval and storage alone. [Sources: https://pubmed.ncbi.nlm.nih.gov/11283309/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC5447931/]
3. [inference] [confidence: high] Selective attention and working memory share overlapping neural machinery, while inhibition of return discourages revisiting already-checked search locations, so effective context management must combine relevance ranking with active suppression of stale, redundant, or previously rejected material. [Sources: https://pmc.ncbi.nlm.nih.gov/articles/PMC6689265/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC6969912/]
4. [inference] [confidence: medium] Predictive-processing evidence implies that higher-order goals and schemas act as priors that determine what counts as meaningful surprise, so AI context assembly should favour deltas, mismatches, and exceptions relative to stable goals instead of repeatedly injecting unchanged background information. [Sources: https://doi.org/10.1038/nrn2787 ; `Research/completed/2026-02-28-predictive-processing-active-inference.md`]
5. [inference] [confidence: high] Hippocampal-prefrontal interactions support the transformation of detailed episodes into reusable schemas, so AI memory should preserve a distinction between episode-level traces and schema-level abstractions and should retain provenance when promoting recurring patterns into reusable context. [Sources: https://pmc.ncbi.nlm.nih.gov/articles/PMC3789138/ ; https://www.frontiersin.org/journals/human-neuroscience/articles/10.3389/fnhum.2023.1217093/full]
6. [inference] [confidence: medium] Dual-process evidence supports a fast-path and slow-path division of labour, so AI systems should default to low-cost schema-driven reasoning for routine cases but switch to slower deliberate evaluation when conflict, ambiguity, or novelty exceeds the fast path's competence. [Sources: https://www.frontiersin.org/journals/psychology/articles/10.3389/fpsyg.2022.805386/full ; https://pmc.ncbi.nlm.nih.gov/articles/PMC5447931/]
7. [inference] [confidence: high] The full evidence set supports a layered context architecture with core always-on goals and values, a small active task buffer, on-demand retrieval of situational evidence, and explicit resolution of conflicts between long-horizon priorities and immediate task demands. [Sources: https://pmc.ncbi.nlm.nih.gov/articles/PMC2864034/ ; https://pubmed.ncbi.nlm.nih.gov/11283309/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC3789138/ ; `Research/completed/2026-03-15-context-layers-aligned-decisions-synthesis.md`]
...
| [inference] Stable priors should stay resident while deltas and conflicts get surfaced. | https://doi.org/10.1038/nrn2787 ; `Research/completed/2026-02-28-predictive-processing-active-inference.md` | medium | Good mechanistic support, but engineering translation remains inferential. |
| [inference] Episode memory and schema memory should be separate layers with promotion between them. | https://pmc.ncbi.nlm.nih.gov/articles/PMC3789138/ ; https://www.frontiersin.org/journals/human-neuroscience/articles/10.3389/fnhum.2023.1217093/full | high | The consolidation literature strongly supports this distinction. |
| [inference] Systems should support fast default reasoning and slow escalation paths. | https://www.frontiersin.org/journals/psychology/articles/10.3389/fpsyg.2022.805386/full ; https://pmc.ncbi.nlm.nih.gov/articles/PMC5447931/ | medium | Supported as a cognitive division of labour; implementation remains approximate. |
| [inference] A layered architecture with core priors, active task state, and on-demand retrieval best matches the full evidence set. | https://pmc.ncbi.nlm.nih.gov/articles/PMC2864034/ ; https://pubmed.ncbi.nlm.nih.gov/11283309/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC3789138/ ; `Research/completed/2026-03-15-context-layers-aligned-decisions-synthesis.md` | high | This integrates the strongest convergent evidence and the downstream synthesis target. |
...
[inference] The evidence was weighed by preferring points that recur across multiple review sources: scarce active capacity, prefrontal top-down control, overlapping attention and working-memory mechanisms, and hippocampal-prefrontal schema consolidation. Those are the least speculative claims in the note. [Sources: https://pmc.ncbi.nlm.nih.gov/articles/PMC2864034/ ; https://pubmed.ncbi.nlm.nih.gov/11283309/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC6689265/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC3789138/]

[inference] The predictive-processing material was used more narrowly because the general predictive-processing mechanism is well supported, while the broadest reading of the Free Energy Principle remains more contested. That is why the design implication is framed as prioritising deltas and conflicts, not as a claim that all intelligent systems must literally minimise free energy. [Sources: https://doi.org/10.1038/nrn2787 ; `Research/completed/2026-02-28-predictive-processing-active-inference.md`]

[inference] The main trade-off exposed by the evidence is between speed and completeness. Fast schema-driven reasoning is efficient but can miss novelty or distort detail, whereas slower controlled reasoning can incorporate conflict and exception handling but consumes scarce active-context capacity. [Sources: https://pmc.ncbi.nlm.nih.gov/articles/PMC2864034/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC3789138/ ; https://www.frontiersin.org/journals/psychology/articles/10.3389/fpsyg.2022.805386/full]

[inference] Competing interpretations were resolved by choosing the most conservative supportable design reading: layered memory, explicit gating, suppression of stale context, and escalation logic are all well grounded, while stronger claims about consciousness, full biological fidelity, or exact neural equivalence are not. [Sources: https://pubmed.ncbi.nlm.nih.gov/11283309/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC6689265/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC3789138/ ; https://www.frontiersin.org/journals/psychology/articles/10.3389/fpsyg.2022.805386/full]

### Risks, Gaps, and Uncertainties

- [fact] The literature cited here supports architectural constraints better than it supports concrete implementation choices such as exact retrieval algorithms or threshold values for escalation. [Sources: https://pmc.ncbi.nlm.nih.gov/articles/PMC2864034/ ; https://pubmed.ncbi.nlm.nih.gov/11283309/ ; https://pmc.ncbi.nlm.nih.gov/articles/PMC3789138/ ; https://www.frontiersin.org/journals/psychology/articles/10.3389/fpsyg.2022.805386/full]
