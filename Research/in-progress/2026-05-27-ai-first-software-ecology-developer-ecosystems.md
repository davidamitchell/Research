...
- 6a. What organisational capabilities differentiate high-AI-ROI organisations from low-AI-ROI organisations?
...
Google's key deployment lessons: (1) UX that blends naturally into workflow outperforms features requiring deliberate triggering; (2) the code author increasingly becomes a reviewer, requiring balance between review cost and added value; (3) rapid A/B iteration (A/B = head-to-head comparison) against usage logs is critical; (4) high-quality data from engineering activities enables model improvement. [fact; source: https://research.google/blog/ai-in-software-engineering-at-google-progress-and-the-path-ahead/]
...
**Organisational lens:** The "Transformation Paradox" (WTI 2026) and the Faros "Acceleration Whiplash" both point to the same systemic failure: tools are deployed without corresponding changes to process, governance, and team cognitive load management. Team Topologies' cognitive load ceiling concept is particularly relevant — if AI tools increase the pace of change faster than teams can absorb, net productivity decreases even if individual lines-per-day increase.

**Economic lens:** The Anthropic Economic Index finding that entry-level engineering tasks face highest automation risk has a compounding effect: fewer junior developers doing routine work means fewer engineers developing the deep codebase familiarity that the METR study shows makes experienced developers most effective on complex tasks. Organisations that automate junior work without rebuilding the pipeline for developing experienced engineers risk a future skills gap.

**Governance/risk lens:** OWASP LLM Top 10, DORA 2025's AI stance requirement, and the SRE shared-fate model all point to the same governance imperative: AI tools must be deployed with explicit policies — what AI can and cannot do without human review, what security gates apply to AI-generated code, and who owns AI-generated code in production incidents. Absent these policies, accountability diffuses in ways that undermine operational resilience.

**Historical lens:** The DORA multi-year research programme showed that organisations took 3-5 years to move from "medium" to "elite" on delivery metrics even with sustained investment. The "Frontier Firm" archetype (WTI 2026) with only 19% of organisations achieving it suggests similar adoption timelines for AI-first operating models.
...
The critical implication is that the primary question for large engineering organisations is not "should we adopt AI tools?" but "what organisational conditions do we need to establish so that AI amplifies our strengths rather than our weaknesses?" DORA's seven capabilities provide the most evidence-grounded framework for diagnosing this readiness gap. [inference; source: https://cloud.google.com/resources/content/2025-dora-ai-assisted-software-development-report]
...
acronym_audit:
  ...
  A/B: expanded at first use in §2.D "A/B = head-to-head comparison"
...
claim_audit: all claims in §2 carry [fact]/[inference]/[assumption] labels
parity_check: §6 Synthesis and Findings are aligned
contradiction_resolution: METR/MIT contradiction resolved; DORA/Faros contradiction resolved
...
The critical implication is that for large engineering organisations, the primary question is not "should we adopt AI tools?" but "what organisational conditions must we establish so that AI amplifies our strengths rather than our weaknesses?" DORA's seven capabilities provide the most evidence-grounded diagnostic framework currently available. [inference; source: https://cloud.google.com/resources/content/2025-dora-ai-assisted-software-development-report]
