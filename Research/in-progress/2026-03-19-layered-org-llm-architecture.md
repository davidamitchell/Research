...
- **[fact]** Real deployments already follow that pattern: Morgan Stanley uses retrieval-augmented GPT-4 over internal knowledge, Harvey uses multi-model routing and legal-task optimisation, and Glean combines a knowledge graph, hybrid search, and domain-adapted models. Sources: `https://www.zenml.io/llmops-database/enterprise-knowledge-management-with-llms-morgan-stanley-s-gpt-4-implementation`; `https://www.harvey.ai/blog/expanding-harveys-model-offerings`; `https://cloud.google.com/blog/products/data-analytics/glean-uses-bigquery-and-google-ai-to-enhance-enterprise-search`; `https://www.glean.com/resources/guides/glean-knowledge-graph`
...
- **[inference]** Concept generation is viable as an enrichment path - especially for ontology extraction and synthetic supervision - but the evidence supports using it to strengthen retrieval or lightweight fine-tuning, not to replace source-grounded retrieval for live enterprise answers.
...
7. **[inference]** Concept generation offers a viable path beyond pure RAG only when it is treated as an enrichment mechanism - ontology extraction, synthetic training data, or concept distillation - rather than as a replacement for retrieving fresh evidence at answer time. Sources: `https://arxiv.org/abs/2306.11644`; `https://arxiv.org/abs/2504.12915`; `Research/completed/2026-03-15-latent-concept-extraction-confluence.md`
8. **[inference]** For a mid-to-large enterprise, the best near-term architecture is a base model plus retrieval and knowledge-graph grounding, optional PEFT for repeated high-value tasks, and verifier layers for regulated or high-cost outputs; training an organisation-specific model from scratch is rarely economically justified. Sources: `https://docs.aws.amazon.com/bedrock/latest/userguide/knowledge-base.html`; `https://cloud.google.com/vertex-ai/generative-ai/docs/rag-overview`; `https://arxiv.org/abs/2106.09685`; `https://arxiv.org/abs/2305.14314`; `https://www.harvey.ai/blog/expanding-harveys-model-offerings`; `https://cloud.google.com/blog/products/data-analytics/glean-uses-bigquery-and-google-ai-to-enhance-enterprise-search`
...
| [inference] Concept generation is most useful as enrichment, not as full retrieval replacement | `https://arxiv.org/abs/2306.11644`; `https://arxiv.org/abs/2504.12915`; `Research/completed/2026-03-15-latent-concept-extraction-confluence.md` | medium | Direct evidence supports components; full architectural claim is synthetic |
| [inference] The best enterprise answer is a layered hybrid stack rather than a standalone organisation-trained model | combined evidence from all rows above | high | Multiple independent evidence streams converge on the same pattern |
...
- **[fact]** The most direct official Morgan Stanley and OpenAI case-study page was not directly fetchable from this environment, so Morgan Stanley-specific details rely on a secondary summary rather than the original page.
- **[inference]** Public vendor material may understate operational failures, so the architectural conclusions should weight the RAG failure literature more heavily than product marketing on claims of completeness.
- **[fact]** None of the consulted public sources provides a clean cost curve for when PEFT overtakes retrieval economics for a given enterprise corpus and query volume.
- **[inference]** Concept generation remains under-evidenced at enterprise deployment scale relative to retrieval and PEFT, even though the component techniques are promising.
- **[fact]** The security and governance implications of training on private enterprise data were out of scope here and remain a separate blocker for some architectures.
...
[inference] Concept generation is viable as an enrichment path - especially for ontology extraction and synthetic supervision - but the evidence supports using it to strengthen retrieval or lightweight fine-tuning, not to replace source-grounded retrieval for live enterprise answers. (Sources: `https://arxiv.org/abs/2306.11644`; `https://arxiv.org/abs/2504.12915`; `Research/completed/2026-03-15-latent-concept-extraction-confluence.md`)
...
7. [inference] Concept generation offers a viable path beyond pure Retrieval-Augmented Generation only when it is treated as an enrichment mechanism - ontology extraction, synthetic training data, or concept distillation - rather than as a replacement for retrieving fresh evidence at answer time. (Sources: `https://arxiv.org/abs/2306.11644`; `https://arxiv.org/abs/2504.12915`; `Research/completed/2026-03-15-latent-concept-extraction-confluence.md`) [confidence: medium]
8. [inference] For a mid-to-large enterprise, the best near-term architecture is a base model plus retrieval and knowledge-graph grounding, optional parameter-efficient fine-tuning for repeated high-value tasks, and verifier layers for regulated or high-cost outputs; training an organisation-specific model from scratch is rarely economically justified. (Sources: `https://docs.aws.amazon.com/bedrock/latest/userguide/knowledge-base.html`; `https://cloud.google.com/vertex-ai/generative-ai/docs/rag-overview`; `https://arxiv.org/abs/2106.09685`; `https://arxiv.org/abs/2305.14314`; `https://www.harvey.ai/blog/expanding-harveys-model-offerings`; `https://cloud.google.com/blog/products/data-analytics/glean-uses-bigquery-and-google-ai-to-enhance-enterprise-search`) [confidence: high]
