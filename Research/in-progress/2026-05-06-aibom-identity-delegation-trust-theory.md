...
- [fact; source: https://openid.net/specs/openid-connect-core-1_0.html; https://openid.net/developers/how-connect-works/] OpenID Connect is decisive for identifying the human principal layer, but it is not a workload-identity model.
- [fact; source: https://www.rfc-editor.org/rfc/rfc8693] RFC 8693 is decisive for distinguishing delegation from impersonation and for expressing current actor state, but it intentionally limits access-control relevance to the current actor and top-level claims.
- [fact; source: https://spiffe.io/docs/latest/spiffe-about/spiffe-concepts/; https://spiffe.io/docs/latest/spiffe-specs/spiffe/] SPIFFE is decisive for workload identity and trust-domain verification, but it does not define end-user delegation or scope manifests.
...
2. **OAuth 2.0 Token Exchange is the best formal basis for AIBOM delegation edges because it names subject, actor, authorized actor, delegation, and impersonation explicitly, but AIBOM must still persist full prior-hop history separately because nested prior actors are informational only for authorization decisions.** ([fact]; high confidence; source: https://www.rfc-editor.org/rfc/rfc8693)
...
| [fact] RFC 8693 provides subject, actor, `act`, `may_act`, delegation, and impersonation semantics, but prior actors remain informational for authorization. | https://www.rfc-editor.org/rfc/rfc8693 | high | Direct standard language. |
