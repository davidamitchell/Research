...
- [x] [Lamport, L. (1978) Time, Clocks, and the Ordering of Events in a Distributed System](https://lamport.azurewebsites.net/pubs/time-clocks.pdf)
- [ ] [Lamport, L. (1978) Time, Clocks, and the Ordering of Events in a Distributed System, DOI record](https://doi.org/10.1145/359545.359563)
...
- Definitions: state space explosion means exponential growth in reachable system states as processes and variables increase; deterministic chaos means deterministic dynamics with sensitive dependence on initial conditions; adversarial example means an input altered by a small worst-case perturbation that changes a model's output.
...
access_note_lorenz_doi: direct DOI landing fetch returned 403 in this session; claims were grounded in the DOI-backed paper abstract reproduced through accessible bibliographic paths plus the American Mathematical Society feature article.
access_note_lamport_doi: direct DOI landing fetch returned 403 in this session; claims were grounded in Lamport's accessible PDF mirror.
access_note_model_checking_book: the seeded MIT Press book page was bibliographic only, so state-explosion claims were grounded instead in Kot's accessible survey note.
...
- [fact] Historical lens: Lorenz's 1963 result and Lamport's 1978 ordering framework both became canonical because they turned intuitive unpredictability problems into formal objects that later work could analyze. [source: https://doi.org/10.1175/1520-0469(1963)020%3C0130:DNF%3E2.0.CO;2; https://lamport.azurewebsites.net/pubs/time-clocks.pdf]
...
In adversarial machine learning, the comparable fragility comes from high-dimensional linear sensitivity and unstable learned boundaries rather than from trajectory divergence through time, so the analogy is real but limited. [inference; source: https://arxiv.org/abs/1412.6572; https://davidamitchell.github.io/Research/research/2026-05-18-rq2-3-predictive-model-fragility.html]
...
4. [fact] Adversarial examples show a comparable sensitivity class in machine learning, because small worst-case perturbations can force high-confidence misclassification and the cited paper attributes much of that vulnerability to linear behavior in high-dimensional spaces. [source: https://arxiv.org/abs/1412.6572] **Confidence:** High.
5. [inference] The strongest formal parallel is that all three domains contain small local perturbations that can trigger large qualitative behavioral changes, even when the underlying update rules are deterministic or tightly specified. [source: https://www.cs.vsb.cz/kot/down/Texts/StateSpace.pdf; https://doi.org/10.1175/1520-0469(1963)020%3C0130:DNF%3E2.0.CO;2; https://arxiv.org/abs/1412.6572] **Confidence:** Medium.
6. [inference] The strongest formal difference is that chaotic sensitivity concerns divergence of nearby trajectories through time, whereas adversarial fragility usually concerns local instability of a learned classifier around a decision boundary at inference time. [source: https://doi.org/10.1175/1520-0469(1963)020%3C0130:DNF%3E2.0.CO;2; https://arxiv.org/abs/1412.6572] **Confidence:** Medium.
7. [fact] Lamport's happened-before relation provides a rigorous partial-order framework for causal ordering in distributed systems, but concurrent events remain those for which neither event happens before the other. [source: https://lamport.azurewebsites.net/pubs/time-clocks.pdf] **Confidence:** High.
...
| [inference] Race conditions operationalize interleaving explosion because different valid schedules can overwrite one another's effects. | https://docs.oracle.com/javase/tutorial/essential/concurrency/interfere.html ; https://cwe.mitre.org/data/definitions/362.html ; https://www.cs.vsb.cz/kot/down/Texts/StateSpace.pdf | high | Lost-update example plus concurrency definition |
...
| [fact] Adversarial examples are small perturbations that can produce high-confidence model errors, with high-dimensional linearity offered as a primary explanation. | https://arxiv.org/abs/1412.6572 | high | Primary adversarial-example paper |
...
The evidence supports a two-part answer. [inference; source: https://www.cs.vsb.cz/kot/down/Texts/StateSpace.pdf; https://doi.org/10.1175/1520-0469(1963)020%3C0130:DNF%3E2.0.CO;2; https://arxiv.org/abs/1412.6572]
First, the analogy is real because all three domains punish small local differences with much larger downstream behavioral differences than naive intuitions expect. [inference; source: https://www.cs.vsb.cz/kot/down/Texts/StateSpace.pdf; https://doi.org/10.1175/1520-0469(1963)020%3C0130:DNF%3E2.0.CO;2; https://arxiv.org/abs/1412.6572]
Second, the mechanisms diverge enough that the analogy must stay bounded: concurrent software fragility is combinatorial, chaotic fragility is dynamical, and adversarial machine-learning fragility is geometric and statistical. [inference; source: https://www.cs.vsb.cz/kot/down/Texts/StateSpace.pdf; https://doi.org/10.1175/1520-0469(1963)020%3C0130:DNF%3E2.0.CO;2; https://arxiv.org/abs/1412.6572]
...
- [fact] The Lorenz source base used here is narrower than the source base for concurrency and adversarial examples, because the direct journal landing page was inaccessible in this session and the argument relies on the DOI-backed paper plus an accessible American Mathematical Society exposition. [source: https://doi.org/10.1175/1520-0469(1963)020%3C0130:DNF%3E2.0.CO;2; https://www.ams.org/publicoutreach/feature-column/fcarc-lorenz]
...
1. **State space explosion in concurrent systems is a genuine exponential-growth problem, because the reachable state space often grows exponentially in the number of processes and variables rather than merely linearly with added components.** ([fact]; high confidence; source: https://www.cs.vsb.cz/kot/down/Texts/StateSpace.pdf)
2. **Race conditions are the runtime symptom of that explosion, because multiple valid interleavings of simple read-modify-write operations can produce different outcomes from the same code and nominal input.** ([inference]; high confidence; source: https://docs.oracle.com/javase/tutorial/essential/concurrency/interfere.html; https://cwe.mitre.org/data/definitions/362.html; https://www.cs.vsb.cz/kot/down/Texts/StateSpace.pdf)
...
4. **Adversarial examples show a comparable sensitivity class in machine learning, because small worst-case perturbations can force high-confidence misclassification and the cited paper attributes much of that vulnerability to linear behavior in high-dimensional spaces.** ([fact]; high confidence; source: https://arxiv.org/abs/1412.6572)
5. **The strongest formal parallel is that all three domains contain small local perturbations that can trigger large qualitative behavioral changes, even when the underlying update rules are deterministic or tightly specified.** ([inference]; medium confidence; source: https://www.cs.vsb.cz/kot/down/Texts/StateSpace.pdf; https://doi.org/10.1175/1520-0469(1963)020%3C0130:DNF%3E2.0.CO;2; https://arxiv.org/abs/1412.6572)
6. **The strongest formal difference is that chaotic sensitivity concerns divergence of nearby trajectories through time, whereas adversarial fragility usually concerns local instability of a learned classifier around a decision boundary at inference time.** ([inference]; medium confidence; source: https://doi.org/10.1175/1520-0469(1963)020%3C0130:DNF%3E2.0.CO;2; https://arxiv.org/abs/1412.6572)
7. **Lamport's happened-before relation provides a rigorous partial-order framework for causal ordering in distributed systems, but concurrent events remain those for which neither event happens before the other.** ([fact]; high confidence; source: https://lamport.azurewebsites.net/pubs/time-clocks.pdf)
