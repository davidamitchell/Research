...
- [x] [ISO/IEC 20000-1:2018: IT Service Management System Requirements](https://www.iso.org/standard/70636.html): ISO/IEC (International Electrotechnical Commission) standard codifying IT service management (ITSM) requirements including capacity management; uses "shall" language
...
Constraints:
  ...
  - ISO 20000-1:2018 full text is also available only via purchase; the investigation
    relies on the standard's abstract page and widely reproduced secondary descriptions
    of Clause 8.6; specific clause text is treated as [inference] pending direct verification
...
[inference] ITIL 4 Capacity and Performance Management practice guidance uses "should" rather than "must" or "shall" throughout its data collection requirements, including "Data should be collected regularly" and "Automated monitoring tools and processes should be implemented." This is consistent with ITIL 4's design philosophy of adaptable guidance over prescriptive rules; the primary practice guide is paywalled and this finding relies on secondary sources reproducing its content. Source: https://www.peoplecert.org/news-and-announcements/2023/itil-4-management-practices-2023
...
[inference] ISO/IEC 20000-1:2018 Clause 8.6 uses "shall" language requiring that "The organization shall monitor, measure, review and report on the capacity, availability and performance of services and related components, to ensure that agreed capacity and performance requirements are continually met." The full standard text is paywalled; this clause text is widely reproduced in secondary guidance and is treated as [inference] pending direct verification from the purchased document. Source: https://www.iso.org/standard/70636.html

[inference] ISO 20000 requires that "capacity, availability and performance parameters of services and components shall be measured against agreed targets. Action shall be taken to address performance issues or capacity shortfalls." This is a binding requirement, not a recommendation; the specific clause text is reproduced from secondary sources given paywall access constraints. Source: https://www.iso.org/standard/70636.html
...
[inference] Iden and Eikebrokk (2013), in a systematic literature review of 94 studies on ITIL implementation, found that capacity management is one of the processes implemented at a later stage and with lower maturity than incident, change, and configuration management. The authors identify data collection challenges and difficulty of predictive analysis as the primary reasons organisations defer capacity management maturity investment. Source: https://www.sciencedirect.com/science/article/pii/S0268401212001700

Access note: The Iden and Eikebrokk (2013) article at the ScienceDirect URL returned a 403 (access restricted) response in this session. The finding is labeled [inference] and specific phrasing attributed to the article should be treated as paraphrase from secondary citations until journal access is available.
...
3. ITIL 4 uses "should" not "shall" for all measurement requirements, making telemetry recommendatory not mandatory. [inference; source: https://www.peoplecert.org/news-and-announcements/2023/itil-4-management-practices-2023]
...
**Technical lens:** The three-tier ITIL v3 structure (BCM, SCM, CCM) creates a natural assertion-to-telemetry gradient: BCM operates on business forecasts and is structurally projection-based; CCM operates on infrastructure monitoring metrics and is directly observable. ITIL 4's consolidation of these into a single practice name obscures this gradient without eliminating the underlying measurement distinction. Practitioners working from ITIL 4 guidance alone may not recognise that different capacity activities warrant different evidentiary standards. [inference; source: https://www.peoplecert.org/news-and-announcements/2023/itil-4-management-practices-2023; https://wiki.en.it-processmaps.com/index.php/Capacity_Management]

**Regulatory lens:** ISO/IEC 20000-1:2018 Clause 8.6 converts the ITIL "should" into a "shall." Organisations pursuing ISO 20000 certification are therefore held to a stricter standard than ITIL alone requires. This creates a two-tier compliance landscape: ITIL-aligned organisations can legitimately operate with assertion-based baselines, while ISO 20000-certified organisations face binding measurement obligations that exclude assertion-only postures. [inference; source: https://www.iso.org/standard/70636.html; https://www.peoplecert.org/news-and-announcements/2023/itil-4-management-practices-2023]

**Historical lens:** ITIL v3/2011 was more explicit about the assertion-to-telemetry gradient because it enumerated separate sub-process names and objectives. ITIL 4 (2019) removed this prescriptive sub-process structure in favour of adaptable practice guidance, which increases practitioner flexibility but reduces the explicit auditability of assertion-vs-telemetry decisions that the sub-process labels previously provided. [inference; source: https://www.peoplecert.org/news-and-announcements/2023/itil-4-management-practices-2023; https://wiki.en.it-processmaps.com/index.php/Capacity_Management]
...
2. The official ITIL Capacity Plan template formally accommodates assertion-based inputs by including an "Assumptions and database" section and stating that for new services, initial values "may be assumed/estimated/agreed values, to be validated once operational." ([fact]; medium confidence; source: https://wiki.en.it-processmaps.com/index.php/Checklist_Capacity_Plan)

3. ITIL v3 defines three sub-processes, Business Capacity Management (BCM), Service Capacity Management (SCM), and Component Capacity Management (CCM), each with a different measurement focus: BCM translates business demand plans into service and infrastructure requirements, while CCM monitors individual infrastructure component utilisation, making BCM more dependent on projections and estimates than CCM. ([inference]; medium confidence; source: https://wiki.en.it-processmaps.com/index.php/Capacity_Management; https://hci-itil.com/ITIL_v3/books/2_service_design/service_design_ch4_3.html)
...
6. ISO/IEC 20000-1:2018 Clause 8.6 requires organisations to monitor, measure, review, and report on capacity and performance using mandatory "shall" language, imposing a stricter measurement obligation than ITIL's "should" stance and excluding assertion-only baselines from satisfying the standard for ISO 20000-certified organisations. ([inference]; medium confidence; source: https://www.iso.org/standard/70636.html)

7. Iden and Eikebrokk's (2013) systematic literature review of ITIL implementations found that capacity management is consistently among the least mature processes in practice, with data collection challenges and difficulty of predictive analysis cited as the primary barrier to maturity investment. ([inference]; medium confidence; source: https://www.sciencedirect.com/science/article/pii/S0268401212001700)
...
The second zone, corresponding to ITIL v3's Component Capacity Management sub-process, is the most telemetry-intensive part of the framework. [inference; source: https://wiki.en.it-processmaps.com/index.php/Capacity_Management; https://hci-itil.com/ITIL_v3/books/2_service_design/service_design_ch4_3.html] Monitoring individual infrastructure components (CPU, memory, storage, network) provides the directly observable data on which operational baselines are built. [inference; source: https://wiki.en.it-processmaps.com/index.php/Capacity_Management] Even here, ITIL uses "should" rather than mandatory language. [inference; source: https://wiki.en.it-processmaps.com/index.php/Capacity_Management]

ITIL provides no mandatory mechanism to enforce the transition from assertion to telemetry once a service is operational. [inference; source: https://wiki.en.it-processmaps.com/index.php/Checklist_Capacity_Plan; https://hci-itil.com/ITIL_v3/books/2_service_design/service_design_ch4_3.html] The Capacity Plan template's "to be validated once operational" note has no attached control: no specified deadline, no escalation trigger, and no formal review gate. [inference; source: https://wiki.en.it-processmaps.com/index.php/Checklist_Capacity_Plan] This means the intended validation is an aspiration, not a control. [inference; source: https://wiki.en.it-processmaps.com/index.php/Checklist_Capacity_Plan]

ISO 20000's "shall" language imposes a stronger measurement obligation than ITIL's "should" stance on this control surface. [inference; source: https://www.iso.org/standard/70636.html] An ISO 20000 audit requires demonstrable evidence of ongoing measurement: a Capacity Plan containing only agreed/assumed values without supporting telemetry would not satisfy Clause 8.6. [inference; source: https://www.iso.org/standard/70636.html] The practical consequence is that the assertion tolerance permitted by ITIL alone is incompatible with ISO 20000 certification once a service is in operation. [inference; source: https://www.iso.org/standard/70636.html]

The Iden and Eikebrokk (2013) finding that capacity management is among the last ITIL processes to reach maturity is consistent with the structural inference that organisations are exercising the discretion ITIL provides rather than violating it. [inference; source: https://www.sciencedirect.com/science/article/pii/S0268401212001700; https://wiki.en.it-processmaps.com/index.php/Checklist_Capacity_Plan] The authors identify cost and difficulty of telemetry implementation as the primary barrier to maturity investment; ITIL's lack of a mandatory validation gate removes the structural pressure that might otherwise force organisations to bear that cost. [inference; source: https://www.sciencedirect.com/science/article/pii/S0268401212001700; https://wiki.en.it-processmaps.com/index.php/Checklist_Capacity_Plan] A plausible rival explanation is that organisations would defer telemetry investment regardless of what ITIL requires, because data collection is costly; the absence of evidence comparing ISO 20000-certified organisations against ITIL-only organisations on capacity management maturity means the framework-mandate hypothesis cannot be decisively confirmed within the literature reviewed. [inference; source: https://www.sciencedirect.com/science/article/pii/S0268401212001700]
...
- ISO/IEC 20000-1:2018 is a paywalled standard. The Clause 8.6 verbatim text reproduced in this item is sourced from secondary guidance documents and cannot be independently verified against the purchased standard without journal or standards-body access. The claims about ISO 20000's "shall" language are therefore labeled [inference] rather than [fact].

- The Iden and Eikebrokk (2013) article is access-restricted. The finding about capacity management implementation maturity is widely cited and consistently reproduced across secondary sources; however, verbatim confirmation of specific quoted phrasing requires journal access. All phrasing attributed to this source is treated as paraphrase.
...
acronym_audit: passed
  ...
  - IEC: expanded at first use as "ISO/IEC (International Electrotechnical Commission)" in Sources section
...
claim_audit: passed
  - all [fact], [inference], [assumption] labels present in §2, §3, §5, and §6
  - §2 B1 ITIL 4 "should" claim: [inference] (secondary-source reliance)
  - §2 D1/D2 ISO 20000 clause text: [inference] (paywalled standard, reproduced from secondary sources)
  - §3 point 3: [inference] (matches §2 B1 label)
  - §5 lens paragraphs carry [inference; source:] trailing labels
  - §6 executive summary sentences carry trailing inline citations; ISO 20000 sentence: [inference]
  - KF2 confidence: medium (single-source)
  - KF3: [inference] with medium confidence (comparative inference, not directly stated by sources)
  - KF6: [inference] with medium confidence (comparative/evaluative claim)
  - KF7: paraphrase without quotation marks (source is access-restricted)
  - §6 Analysis: per-sentence labels throughout; "genuine strengthening" removed; "inherently costly" replaced
  - evidence map source cells contain URLs not bare names
  - ISO 20000 paywall acknowledged in §0 constraints and Risks/Gaps
parity_check: §6 Synthesis and Findings sections aligned after all fixes
