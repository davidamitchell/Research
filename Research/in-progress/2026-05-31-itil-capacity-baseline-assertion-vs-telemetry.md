...
The Iden and Eikebrokk (2013) finding that capacity management is among the last ITIL processes to reach maturity, with data collection challenges as the primary barrier, is consistent with the structural inference that organisations are rationally exercising the discretion ITIL provides rather than violating it. The primary driver the authors identify is the cost and difficulty of telemetry implementation, not the absence of a mandate; ITIL's lack of a mandatory validation gate removes the structural pressure that might otherwise force organisations to bear that cost. A rival explanation is that organisations would defer telemetry investment regardless of what ITIL requires, because data collection is inherently costly; the absence of evidence that ISO 20000-certified organisations achieve higher capacity management maturity than ITIL-only organisations would support that rival explanation, but this comparison is outside the scope of the literature reviewed. [inference; source: https://www.sciencedirect.com/science/article/pii/S0268401212001700; https://wiki.en.it-processmaps.com/index.php/Checklist_Capacity_Plan]
...
claim_audit: passed
  ...
  - §2 B1 ITIL 4 "should" claim updated from [fact] to [inference] reflecting secondary-source reliance
  - all §5 lens paragraphs carry [inference; source:] trailing labels
...
parity_check: §6 Synthesis and Findings sections are consistent; repetition in Analysis removed; alternative-explanations paragraph added
